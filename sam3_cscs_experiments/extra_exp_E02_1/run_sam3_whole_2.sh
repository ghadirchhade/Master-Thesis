...
#  run_sam3_whole_2.sh  ==>  dispatcher for the SAM3 WHOLE-IMAGE (no tiling) Rumex
#  experiment (E02_1_b)
#
#  Sets up the environment, decides where everything lives under $SCRATCH, and launches
#  infer_sam3_whole_2.py -- the port of the E02_1 Colab notebook: NO tiling, the whole
#  image resized to MAX_DIM (1024) on its long side, THREE exemplar BOXES passed straight
#  through as prompts (no pasted strip), one forward pass per anchor, no NMS.
...
EXPERIMENT_NAME="${EXPERIMENT_NAME:-E02_1_b}"
...
PRESET="${PRESET:-multi}"               # multi = 3 exemplar boxes (E02_1_b), single = 1
MAX_DIM="${MAX_DIM:-1024}"              # notebook value: long side the image is resized to
THRESHOLD="${THRESHOLD:-0.4}"           # SAM3 confidence threshold (0.4 here, 0.3 in the tiled runs)
MASK_THRESHOLD="${MASK_THRESHOLD:-0.5}" # mask binarisation, as in the notebook
IOU_THRESHOLD="${IOU_THRESHOLD:-0.5}"   # TP matching threshold
NMS_IOU="${NMS_IOU:-0}"                 # 0 = OFF. No tiling -> no tile-boundary duplicates
DTYPE="${DTYPE:-float32}"
SAVE_MASKS="${SAVE_MASKS:-0}"
CACHE_RESIZED="${CACHE_RESIZED:-1}"     # 1 = resize once per image and reuse for every anchor
...
echo "tiling=OFF  max_dim=${MAX_DIM}  conf=${THRESHOLD}  mask=${MASK_THRESHOLD}  nms_iou=${NMS_IOU}"
echo "cache_resized=${CACHE_RESIZED}"
...
try:
    ...
    print("  -> inference will refuse to start. Run './run_sam3_whole_2.sh download' on a login node.")
PY
echo "========================"
...
if [[ "${CACHE_RESIZED}" == "1" ]]; then
    COMMON_ARGS+=(--cache-resized)
else
    COMMON_ARGS+=(--no-cache-resized)
fi
...
PY
        ...
        echo "      with: ./run_sam3_whole_2.sh dryrun   (the PYTHON DEBUG block prints it)."
        ;;

    dryrun)
        # No model, no GPU: just proves the dataset layout is understood and prints the cost.
        python "${SCRIPT_DIR}/infer_sam3_whole_2.py" "${COMMON_ARGS[@]}" --dry-run "$@"
        ;;
    ...
    run)
        ...
        if [[ ! -d "${HF_HOME}" || -z "$(ls -A "${HF_HOME}" 2>/dev/null)" ]]; then
            ...
            echo "         Run './run_sam3_whole_2.sh download' on a login node first."
        fi
        ...
        for ((i = 0; i < NUM_GPUS; i++)); do
            ...
            python "${SCRIPT_DIR}/infer_sam3_whole_2.py" \
                "${COMMON_ARGS[@]}" \
                --num-shards "${NUM_GPUS}" \
                --shard-index "${i}" \
                --no-aggregate \
