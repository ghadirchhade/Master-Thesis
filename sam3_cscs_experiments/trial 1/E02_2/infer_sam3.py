...
"""
DATASET:
-The dataset root contains four archives; we deliberately use only two of them: AGS_Multiple_Fields && AgsSpringRumex
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="SAM3 exemplar-prompted Rumex detection benchmark (inference + evaluation).",
    ...
    g.add_argument("--experiment-name", default="E02_2",
                   help="Free-form tag written into every CSV row and into the file names.")
    g.add_argument("--preset", choices=["exp1", "exp2", "exp3"], default=None,
                   help="Convenience: exp1 = tiling + 3 exemplars, exp2 = NO tiling + 3 exemplars, "
                        "exp3 = tiling + 1 exemplar. Explicit flags below always win.")
    g.add_argument("--n-exemplars", type=int, default=None,
                   help="Number of exemplar crops per prompt (anchor + n-1 sampled others). Default 3.")
    tiling = g.add_mutually_exclusive_group()
    tiling.add_argument("--tiling", dest="use_tiling", action="store_true", default=None,
                        help="Split the image into overlapping tiles (default).")
    tiling.add_argument("--no-tiling", dest="use_tiling", action="store_false",
                        help="Feed the whole image to SAM3 as one single tile (Exp2 ablation).")
    ...
    preset_table = {                       # (n_exemplars, use_tiling)
        "exp1": (3, True),
        "exp2": (3, False),
        "exp3": (1, True),
    }
    preset_n, preset_tiling = preset_table.get(args.preset, (3, True))
    if args.n_exemplars is None:
        args.n_exemplars = preset_n
    if args.use_tiling is None:
        args.use_tiling = preset_tiling
...
def run_sam3_pipeline(runner: Sam3Runner, image: Image.Image, exemplar_crops: Sequence[Image.Image],
    ...
    if args.use_tiling:
        tiles = tile_bboxes(image.width, image.height, args.tile_size, args.overlap)
        ...
        # Whole image as a single tile
        composed, crop_boxes, offset = compose_tile_with_exemplars(
            image, exemplar_crops, margin=args.margin, feather_width=args.feather_width,
        )
        boxes, scores, masks = runner.run_on_tile(
            composed, crop_boxes, offset,
            threshold=args.threshold, mask_threshold=args.mask_threshold,
        ...
        for b, s, m in zip(boxes, scores, masks):
            ...
            all_scores.append(float(s))
            all_masks.append((np.asarray(m), 0, 0) if keep_masks else None)
        del boxes, scores, masks
...
SUPERVISION_HINT = (
    ...
    "  then point the container's PYTHONPATH at $SCRATCH/pyextra (see HOW_TO_RUN.md).\n"
...
def main() -> None:
    ...
    print("=" * 92)
    print(f" SAM3 EXEMPLAR BENCHMARK | experiment={exp} | shard {args.shard_index + 1}/{args.num_shards}")
    print("=" * 92)
        ...
        for img_idx, rec in enumerate(my_records, start=1):
            ...
            for anchor_idx in anchors:
                ...
                boxes, scores, masks = run_sam3_pipeline(
                    runner, image, exemplar_crops, args, keep_masks=args.save_masks
                )
