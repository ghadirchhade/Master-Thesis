...
#  run_sam3_single.sh  ==>  dispatcher for the SAM3 TILED SINGLE-BOX Rumex experiment (E01_2)
#
#  Sets up the environment, decides where everything lives under $SCRATCH, and launches
#  infer_sam3_single.py -- the port of the E01_2 Colab notebook: tiling ON, but the prompt
#  is ONE exemplar crop (the anchor alone) instead of three. No tile batching.
#
#  MODES
...
PRESET="${PRESET:-single}"              # single = 1 exemplar crop (E01_2), multi = 3
...
NMS_IOU="${NMS_IOU:-0.5}"               # merging detections across overlapping tiles
DTYPE="${DTYPE:-float32}"
...
try:
    ...
    print("  -> inference will refuse to start. Run './run_sam3_single.sh download' on a login node.")
PY
echo "========================"
...
PY
        ...
        echo "      with: ./run_sam3_single.sh dryrun   (the PYTHON DEBUG block prints the result)."
        ;;
 
    dryrun)
        # No model, no GPU: just proves the dataset layout is understood and prints the cost.
        python "${SCRIPT_DIR}/infer_sam3_single.py" "${COMMON_ARGS[@]}" --dry-run "$@"
        ;;
    ...
    run)
        ...
        if [[ ! -d "${HF_HOME}" || -z "$(ls -A "${HF_HOME}" 2>/dev/null)" ]]; then
            ...
            echo "         Run './run_sam3_single.sh download' on a login node first."
        fi
        ...
        for ((i = 0; i < NUM_GPUS; i++)); do
            ...
            python "${SCRIPT_DIR}/infer_sam3_single.py" \
                "${COMMON_ARGS[@]}" \
                --num-shards "${NUM_GPUS}" \
                --shard-index "${i}" \
                --no-aggregate \
