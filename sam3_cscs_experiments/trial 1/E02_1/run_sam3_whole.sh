...
#  run_sam3_whole.sh  ==>  dispatcher for the SAM3 WHOLE-IMAGE Rumex experiment (E02_1)

#  Sets up the environment, decides where everything lives under $SCRATCH, and launches
#  infer_sam3_whole.py -- GT boxes are handed to SAM3
#  directly as box prompts on the whole image resized to MAX_DIM. No exemplar strip, no
#  tiling, no NMS.
#
#  MODES
...
PRESET="${PRESET:-multi}"               # multi = 3 exemplar boxes (notebook), single = 1
MAX_DIM="${MAX_DIM:-1024}"              # longest side after resize; the notebook used 1024
...
echo "max_dim=${MAX_DIM}  mask_threshold=${MASK_THRESHOLD}"
...
try:
    ...
    print("  -> inference will refuse to start. Run './run_sam3_whole.sh download' on a login node.")
PY
echo "========================"
...
PY
        ...
        echo "      with: ./run_sam3_whole.sh dryrun   (the PYTHON DEBUG block prints the result)."
        ;;
 
    dryrun)
        # No model, no GPU: just proves the dataset layout is understood and prints the cost.
        python "${SCRIPT_DIR}/infer_sam3_whole.py" "${COMMON_ARGS[@]}" --dry-run "$@"
        ;;
    ...
    run)
        ...
        if [[ ! -d "${HF_HOME}" || -z "$(ls -A "${HF_HOME}" 2>/dev/null)" ]]; then
            ...
            echo "         Run './run_sam3_whole.sh download' on a login node first."
        fi
        ...
        for ((i = 0; i < NUM_GPUS; i++)); do
            ...
            python "${SCRIPT_DIR}/infer_sam3_whole.py" \
                "${COMMON_ARGS[@]}" \
                --num-shards "${NUM_GPUS}" \
                --shard-index "${i}" \
                --no-aggregate \
