...
# -*- coding: utf-8 -*-
"""
TILED SINGLE-BOX SAM3 EXEMPLAR + GLOBAL DOWNSCALE PASS

E01_2_c is the ONE-exemplar variant WITH an extra whole-image context pass, run on a FINER
tile grid: tiling ON (1536 px tiles, 384 px overlap = 25%), the prompt is a SINGLE crop
(the anchor box alone), and on top of the tiled sweep the whole image is also run once at
1/GLOBAL_DOWNSCALE resolution. The two detection sets are then merged by a single global
NMS.
...
CSV_COLUMNS = [
    ...
    "n_pred_tiled",     # raw detections contributed by the tiled sweep    (E01_2_c extra)
    "n_pred_global",    # raw detections contributed by the global pass    (E01_2_c extra)
    "image_width",
    "image_height",
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="SAM3 tiled, single-exemplar Rumex detection benchmark with an extra "
                    "downscaled whole-image pass, on a 1536/384 tile grid (inference + "
                    "evaluation). Port of the E01_2 Colab notebook.",
    ...
    g.add_argument("--experiment-name", default="E01_2_c",
    ...
    g.add_argument("--preset", choices=["single", "multi"], default=None,
                   help="Convenience: single = 1 exemplar crop, the anchor alone (E01_2_c, "
                        "the default), multi = 3 exemplar crops (reproduces E02_2_c's prompt "
                        "with this script). --n-exemplars always wins over the preset.")
    ...
    g.add_argument("--tile-size", type=int, default=1536,
                   help="Notebook value for E01_2_c. Still comfortably larger than the widest "
    ...
    g.add_argument("--overlap", type=int, default=384,
                   help="Notebook value for E01_2_c (25%% of the tile). NOTE this is BELOW the "
                        "widest annotated plant (706 px), so a very large plant on a tile "
                        "boundary can be cut in every tile containing it -- unlike E01_2_b's "
...
def run_sam3_pipeline(runner: Sam3Runner, image: Image.Image, exemplar_crops: Sequence[Image.Image],
    ...
    #
    # No batching: one composed tile per forward pass, as in the other ported scripts.
...
SUPERVISION_HINT = (
    ...
    "  then point the container's PYTHONPATH at $SCRATCH/pyextra (see HOW_TO_RUN_single_3.md).\n"
...
def main() -> None:
    ...
    print("=" * 92)
    print(f" SAM3 TILED SINGLE-BOX + GLOBAL PASS BENCHMARK | experiment={exp} | "
