# HOW TO RUN — Experiment E01_2_c

**What E01_2_c is:** SAM3 prompted with **1 exemplar crop** (the anchor box, alone), **tiling ON at 1536 px tiles / 384 px overlap (25%)**, **plus one extra forward pass on the whole image downscaled by 2**, everything merged by a single NMS. Run over both `AGS_Multi_Rumex` and `AgsSpringRumex` pooled as one dataset. Confidence threshold 0.3, NMS IoU 0.5.

This is the fourth cell of a 2×2. All four share the global pass, the thresholds, the metrics and the exemplar seeding — only the prompt size and the tile geometry vary:

| | 2000 / 800 tiles | 1536 / 384 tiles |
|---|---|---|
| **1 crop** | E01_2_b | **E01_2_c** ← this file |
| **3 crops** | E02_2_b | E02_2_c |

- **E01_2_c vs E02_2_c** — exemplar count, on the fine grid
- **E01_2_c vs E01_2_b** — tile geometry, with one crop

Anything diagonal changes two things at once. See *"The comparisons this experiment exists for"* below.

This is the CSCS port of the E01_2 Colab notebook with `TILE_SIZE = 1536` / `OVERLAP = 384`. Nothing here overwrites the other experiments: every script carries a `_single_3` suffix and the experiment is tagged `E01_2_c`.
...
├── run_sam3_3.sh             ← E02_2_c, untouched
├── submit_sam3_3.sh          ← E02_2_c, untouched
├── infer_sam3_single_3.py    ← NEW  (E01_2_c)
├── run_sam3_single_3.sh      ← NEW
├── submit_sam3_single_3.sh   ← NEW
└── HOW_TO_RUN_single_3.md    ← this file
```
...
chmod +x run_sam3_single_3.sh submit_sam3_single_3.sh
ls -l
```

**You should see:** `run_sam3_single_3.sh` and `submit_sam3_single_3.sh` marked executable (`-rwxr-xr-x`).
...
These six lines **are** E01_2_c. If `n_exemplars` says 3, set `PRESET=single`. If `tile=2000, overlap=800`, you are running E01_2_b's geometry — `TILE_SIZE`/`OVERLAP` leaked in from your shell.
...
**35** is the right number for tile=1536 / overlap=384 on a 8192×5460 image. If you see 28, the geometry has been overridden to E01_2_b's values.
...
Still inside the interactive shell. This is the first time the model actually loads. It writes to a throwaway folder so it cannot pollute the real E01_2_c results.

```bash
EXPERIMENT_NAME=E01_2_c_smoke \
OUTPUT_DIR=$SCRATCH/experiments/sam3/_smoke_single_3 \
NUM_GPUS=1 \
./run_sam3_single_3.sh run --limit-images 2
```
...
SAM3 ready. Parameters live on: cuda:0
    ...
    [E01_2_c_smoke] run #1 | AGS_Multi_Rumex/2022.../DJI_0001 | anchor=0 (1/12) | prompt=0 | tiles=35 | preds=8 (tiled=13, global=2) | mAP50=0.327 | 29.1s
```

Five things to note from this output:

1. **The `NOTE:` line** — how many ground-truth boxes in this image are bigger than the 384 px overlap. This is the fine-grid risk (see below). It is printed once per image, log only, and changes nothing. If it fires on most images, that is a real argument for a larger `OVERLAP`.
2. **`prompt=0`** — a single number, no `+`. That is the whole point of E01_2_c: one exemplar, the anchor itself. If you see `prompt=0+7+3`, you are running the `multi` preset.
3. **`tiles=35`** — the finer grid. If it says 28, the geometry is wrong.
4. **`tiled=13, global=2`** — how many raw detections each source contributed *before* the merge. If `global=` is 0 on every single run, the global pass is finding nothing and you may as well turn it off.
5. **`29.1s`** (or whatever you get) is the time for **ONE** anchor run. Multiply by the total anchor count from Step 6c to estimate the whole job. If that exceeds 24 h, see *"If it will not fit in 24 hours"* below — the short answer is that it is fine, you just resubmit and it resumes.
...
The `exemplar strip ... is wider than the tile` note is **rare here** — with one crop it needs a single plant wider than about 1520 px. That is the one place E01_2_c is cleaner than E02_2_c, where three crops side by side overflow a 1536 px tile fairly often.
...
```bash
cd $HOME/2025-OverneyTechnologiesProject/7_cscs_experiments/sam3
sbatch submit_sam3_single_3.sh
```

**You should see:** `Submitted batch job 1234567`

That is it. No arguments needed — E01_2_c is the default. The job asks for 1 node, 4 GPUs, 64 CPUs, 24 hours, and internally splits the images across the 4 GPUs.

All the experiments can queue at the same time: they write to different folders (`.../sam3/E01_2`, `.../sam3/E02_1`, `.../sam3/E02_2`, `.../sam3/E02_2_b`, `.../sam3/E01_2_b`, `.../sam3/E02_2_c`, `.../sam3/E01_2_c`) and different log files (`sam3_single_%j.out`, `sam3_whole_%j.out`, `sam3_%j.out`, `sam3_2_%j.out`, `sam3_single_2_%j.out`, `sam3_3_%j.out`, `sam3_single_3_%j.out`).
...
tail -f sam3_single_3_1234567.out                     # everything, live
tail -f $SCRATCH/experiments/sam3/E01_2_c/shard0.log  # just GPU 0
```

Count how many runs are finished so far:

```bash
cat $SCRATCH/experiments/sam3/E01_2_c/runs/results_E01_2_c_shard*.csv | grep -c E01_2_c
```

Count how often the oversized-plant note fired:

```bash
grep -c "larger than the tile overlap" sam3_single_3_1234567.out
```
...
Everything lands in `$SCRATCH/experiments/sam3/E01_2_c/`:

```bash
ls $SCRATCH/experiments/sam3/E01_2_c/
```
...
| File | What it is |
|---|---|
| `summary_E01_2_c_all.csv` | the headline numbers — mean and std of mAP50 / precision / recall / IoU1 / IoU2, averaged over images |
| `summary_E01_2_c_AGS_Multi_Rumex.csv` | same, for that archive only |
| `summary_E01_2_c_AgsSpringRumex.csv` | same, for that archive only |

Look at them:

```bash
cd $SCRATCH/experiments/sam3/E01_2_c
column -s, -t < summary_E01_2_c_all.csv
```
...
| File | What it is |
|---|---|
| `results_E01_2_c_all.csv` | one row per (image × anchor) — the raw data |
| `results_E01_2_c_<archive>.csv` | the same rows split by archive |
| `image_level_E01_2_c_*.csv` | one row per image (the prompt runs averaged together) |
| `raw_summary_E01_2_c_*.csv` | mean/std treating every run as an independent sample |
| `runs/results_E01_2_c_shard*.csv` | the per-GPU raw output; this is what resume reads |
| `runs/detections_E01_2_c_shard*.jsonl` | the predicted boxes + scores for every run |
| `runs/masks_E01_2_c_shard*.jsonl` | only with `SAVE_MASKS=1` |
| `run_config_E01_2_c.json` | every parameter used, for the thesis appendix |
...
```
Resume: 4821 run(s) already recorded for E01_2_c; they will be skipped.
```
...
2. **`DTYPE=bfloat16 sbatch submit_sam3_single_3.sh`** — roughly 2× faster on GH200/H100, negligible accuracy change.
...
then change the one line at the bottom of `submit_sam3_single_3.sh`:
...
| `CUDA out of memory` | this is the smallest composite of the four experiments (one crop, 1536 px tile), so it is the least likely place to hit it. `DTYPE=bfloat16` first, then `TILE_SIZE=1280 OVERLAP=320`, then `NUM_GPUS=2`. Note there is no batch auto-split any more — batching was removed |
...
| `tiles=28` instead of `tiles=35` | `TILE_SIZE`/`OVERLAP` have been overridden to E01_2_b's values |
...
| merge with another experiment drops rows | the two runs did not cover the same images — check `--archives` / `--limit-images` on both |
...
```bash
DTYPE=bfloat16          sbatch submit_sam3_single_3.sh   # faster
THRESHOLD=0.001         sbatch submit_sam3_single_3.sh   # proper mAP curve
OVERLAP=768             sbatch submit_sam3_single_3.sh   # overlap clears the widest plant (706px)
GLOBAL_PASS=0           sbatch submit_sam3_single_3.sh   # tiles only (ablate the global pass)
GLOBAL_DOWNSCALE=4      sbatch submit_sam3_single_3.sh   # coarser global pass
PRESET=multi            sbatch submit_sam3_single_3.sh   # 3 exemplar crops instead of 1
NMS_IOU=0.3             sbatch submit_sam3_single_3.sh   # stricter duplicate merging
TILE_SIZE=1280 OVERLAP=320 sbatch submit_sam3_single_3.sh  # even finer grid
CACHE_TILES=0           sbatch submit_sam3_single_3.sh   # lower host RAM
NUM_GPUS=2              sbatch submit_sam3_single_3.sh   # fewer GPUs
SAVE_MASKS=1            sbatch submit_sam3_single_3.sh   # also store masks (large!)
DATASET_ROOT=/some/path sbatch submit_sam3_single_3.sh   # dataset elsewhere
```

Anything after the mode is forwarded straight to Python, so one-off flags work too:

```bash
./run_sam3_single_3.sh run --limit-images 20 --max-anchors-per-image 3
./run_sam3_single_3.sh run --archives AGS_Multi_Rumex        # one archive only
./run_sam3_single_3.sh run --no-global-pass                  # same as GLOBAL_PASS=0
```
