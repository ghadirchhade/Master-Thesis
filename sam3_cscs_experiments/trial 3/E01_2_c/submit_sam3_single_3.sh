...
#  submit_sam3_single_3.sh  --  SLURM batch script for the SAM3 TILED SINGLE-BOX + GLOBAL
#  PASS Rumex experiment (E01_2_c: 1 exemplar crop, tiling ON at 1536/384, one extra
#  downscaled whole-image pass, no batching)

#SBATCH --no-requeue
#SBATCH --account="go077"
#SBATCH --job-name="sam3_e01_2c"
#SBATCH --output=sam3_single_3_%j.out
#SBATCH --error=sam3_single_3_%j.err
...
# plus the global pass = 36 forward passes per anchor, i.e. ~25% more passes than E01_2_b
# (28 + 1) -- though each pass is cheaper, since a one-crop strip on a 1536 px tile makes
# the smallest composite of any of these four runs. Expect a comparable wall-clock. If it
# overruns, just resubmit -- the job resumes from the shard CSVs.
...
chmod +x "${SCRIPT_DIR}/run_sam3_single_3.sh"

# Defaults for this job; any of these can be overridden from the submitting shell.
export EXPERIMENT_NAME="${EXPERIMENT_NAME:-E01_2_c}"
...
# run_sam3_single_3.sh sets. Do NOT rely on the compute node reaching PyPI -- it cannot. If
# the image's transformers is too old for Sam3Model, build a dedicated image
# (HOW_TO_RUN_single_3.md, "Problem A").
srun \
    --environment=yolo26 \
    --container-workdir="$PWD" \
    --cpu-bind=cores \
    bash -c "${SCRIPT_DIR}/run_sam3_single_3.sh run"