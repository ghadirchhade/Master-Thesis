...
#  run_sam3_3.sh  ==>  dispatcher for the SAM3 TILED MULTI-BOX + GLOBAL PASS Rumex
#  experiment (E02_2_c)
#
#  Sets up the environment, decides where everything lives under $SCRATCH, and launches
#  infer_sam3_3.py -- the port of the E02_2 Colab notebook: tiling ON (1536 px tiles,
#  384 px overlap), THREE exemplar crops in the strip, plus one extra forward pass on the
...
EXPERIMENT_NAME="${EXPERIMENT_NAME:-E02_2_c}"
...
PRESET="${PRESET:-multi}"               # multi = 3 exemplar crops (E02_2_c), single = 1
TILE_SIZE="${TILE_SIZE:-1536}"          # notebook value (E02_2_c)
...
try:
    ...
    print("  -> inference will refuse to start. Run './run_sam3_3.sh download' on a login node.")
PY
echo "========================"
...
PY
        ...
        echo "      with: ./run_sam3_3.sh dryrun   (the PYTHON DEBUG block prints the result)."
        ;;

    dryrun)
        # No model, no GPU: just proves the dataset layout is understood and prints the cost.
        python "${SCRIPT_DIR}/infer_sam3_3.py" "${COMMON_ARGS[@]}" --dry-run "$@"
        ;;
    ...
    run)
        ...
        if [[ ! -d "${HF_HOME}" || -z "$(ls -A "${HF_HOME}" 2>/dev/null)" ]]; then
            ...
            echo "         Run './run_sam3_3.sh download' on a login node first."
        fi
        ...
        for ((i = 0; i < NUM_GPUS; i++)); do
            ...
            python "${SCRIPT_DIR}/infer_sam3_3.py" \
                "${COMMON_ARGS[@]}" \
                --num-shards "${NUM_GPUS}" \
                --shard-index "${i}" \
                --no-aggregate \
