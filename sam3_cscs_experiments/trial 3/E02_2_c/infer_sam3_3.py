...
# -*- coding: utf-8 -*-
"""
TILED MULTI-BOX SAM3 EXEMPLAR + GLOBAL DOWNSCALE PASS

E02_2_c is the three-exemplar variant WITH an extra whole-image context pass, run on a
FINER tile grid: tiling ON (1536 px tiles, 384 px overlap = 25%), the prompt is THREE
crops (the anchor box plus two other ground-truth boxes from the same image), and on top
of the tiled sweep the whole image is also run once at 1/GLOBAL_DOWNSCALE resolution. The
two detection sets are then merged by a single global NMS.
...
For every image, and then for every ground-truth box in that image ("anchor"):
  ...
  2. TILING                   (tile_bboxes)
     ...
     At the defaults (1536 / 384) a 8192x5460 image becomes 35 tiles.
  ...
  8. GLOBAL DOWNSCALE PASS    (run_sam3_pipeline, --global-pass)
     The whole image is resized by 1/GLOBAL_DOWNSCALE, the same exemplar strip is pasted
     above it, one extra forward pass is run, steps 5-6 are applied with the small image
     as the "tile", and the surviving boxes are multiplied back up to full-image pixels.

  9. NMS MERGE                (nms_merge)
     Because tiles overlap -- and because the global pass re-finds plants the tiles
     already have -- the same plant is detected several times. ONE global greedy NMS
     over every detection of the image (tiled + global) removes the duplicates.

 10. METRICS                  (compute_detection_metrics)
...
CSV_COLUMNS = [
    ...
    "source_class_id",
    "n_gt",
    "n_pred",           # after the global NMS -- the number the metrics are computed on
    "n_pred_tiled",     # raw detections contributed by the tiled sweep    (E02_2_c extra)
    "n_pred_global",    # raw detections contributed by the global pass    (E02_2_c extra)
    "image_width",
    "image_height",
    "n_tiles",          # tiles this image was cut into
    "global_pass",      # 1 / 0, so a mixed folder can still be split later
    "global_downscale",
    "runtime_s",
]
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="SAM3 tiled, three-exemplar Rumex detection benchmark with an extra "
                    "downscaled whole-image pass, on a 1536/384 tile grid (inference + "
                    "evaluation). Port of the E02_2 Colab notebook.",
    ...
    g.add_argument("--experiment-name", default="E02_2_c",
    ...
    g.add_argument("--preset", choices=["single", "multi"], default=None,
                   help="Convenience: multi = 3 exemplar crops (E02_2_c, the default), "
                        "single = 1 exemplar crop, the anchor alone (the E01_2_b prompt, "
                        "on this tile grid). --n-exemplars always wins over the preset.")
    ...
    g.add_argument("--tile-size", type=int, default=1536,
                   help="Notebook value for E02_2_c. Still comfortably larger than the widest "
                        "annotated plant (706 px), and small enough that SAM3's internal resize "
                        "to ~1008 px costs less detail than a 2000 px tile would.")
    g.add_argument("--overlap", type=int, default=384,
                   help="Notebook value for E02_2_c (25%% of the tile). NOTE this is BELOW the "
                        "widest annotated plant (706 px), so a very large plant on a tile "
                        "boundary can be cut in every tile containing it -- unlike E02_2_b's "
                        "800 px. Raise it if that shows up in the results.")
    ...
    # the extra whole-image pass
    g = p.add_argument_group("global downscale pass")
    gp = g.add_mutually_exclusive_group()
    gp.add_argument("--global-pass", dest="global_pass", action="store_true", default=True,
                    help="Also run ONE forward pass on the whole image downscaled by "
                         "--global-downscale, and merge its detections with the tiled ones "
                         "(the notebook's ADD_GLOBAL_CONTEXT_PASS=True). Default: on.")
    gp.add_argument("--no-global-pass", dest="global_pass", action="store_false",
                    help="Tiles only. This makes the run directly comparable to E01_2/E02_2 "
                         "on the exemplar count alone.")
    g.add_argument("--global-downscale", type=int, default=2,
                   help="Shrink factor for the global pass (2 = half width and half height).")
    ...
    cache.add_argument("--cache-tiles", dest="cache_tiles", action="store_true", default=True,
                       help="Crop the tiles once per image and reuse them for every anchor of "
                            "that image (the notebook's optimisation; 35 crops instead of "
                            "35 x n_anchors). Costs ~250 MB of RAM at tile-size 1536.")
...
def strip_content_width(crop_images: Sequence[Image.Image], margin: int) -> int:
    # Same arithmetic as compose_tile_with_exemplars, exposed so the caller can warn ONCE
    # per anchor instead of once per tile (35 identical warnings help nobody).
    return sum(c.width for c in crop_images) + margin * (len(crop_images) + 1)
...
def run_sam3_pipeline(runner: Sam3Runner, image: Image.Image, exemplar_crops: Sequence[Image.Image],
    ...
    # Returns (boxes, scores, masks, stats) with boxes in FULL-IMAGE coordinates, after NMS.
    # `stats` counts what each source contributed BEFORE the merge, so you can tell from the
    # CSV whether the global pass is actually doing anything.
    #
    # No batching: one composed tile per forward pass, as in infer_sam3_single.py /
    # infer_sam3_2.py.
    ...
    for (x1, y1, x2, y2, cached_tile) in tiles:
        ...
        for b, s, m in zip(boxes, scores, masks):
            ...
            # (mask, origin_x, origin_y, scale): scale 1.0 -> the mask is already at full res
            all_masks.append((np.asarray(m), x1, y1, 1.0) if keep_masks else None)
        del boxes, scores, masks, composed
        if cached_tile is None:
            del tile

    n_tiled = len(all_boxes)
    n_global = 0

    #  2. global downscaled context pass 
    # One extra forward pass on the WHOLE image, shrunk by --global-downscale. The exemplar
    # crops are pasted at full resolution (notebook behaviour, see the module docstring).
    if args.global_pass and args.global_downscale > 1:
        small_w = max(1, image.width // args.global_downscale)
        small_h = max(1, image.height // args.global_downscale)
        small_img = image.resize((small_w, small_h))
        g_composed, g_crop_boxes, g_offset = compose_tile_with_exemplars(
            small_img, exemplar_crops, margin=args.margin, feather_width=args.feather_width,
            warn_wide=False,
        )
        g_boxes, g_scores, g_masks = runner.run_on_tile(
            g_composed, g_crop_boxes, g_offset,
            threshold=args.threshold, mask_threshold=args.mask_threshold,
        )
        g_boxes, g_scores, g_masks = filter_implausible_boxes(
            g_boxes, g_scores, g_masks, small_w, small_h,
            min_fill_ratio=args.min_fill_ratio,
            max_area_fraction=args.max_area_fraction,
            edge_margin=args.edge_margin,
        )
        # small-image coordinates -> full-image coordinates
        scale = image.width / float(small_w)
        for b, s, m in zip(g_boxes, g_scores, g_masks):
            all_boxes.append([b[0] * scale, b[1] * scale, b[2] * scale, b[3] * scale])
            all_scores.append(float(s))
            # the mask stays at small-image resolution; `scale` says how to blow it back up
            all_masks.append((np.asarray(m), 0, 0, scale) if keep_masks else None)
        n_global = len(g_boxes)
        del g_boxes, g_scores, g_masks, g_composed
        small_img.close()
        gc.collect()

    #  3. one global NMS over tiled + global detections 
    boxes, scores, masks = nms_merge(all_boxes, all_scores, all_masks, iou_thresh=args.nms_iou)
    return boxes, scores, masks, {"n_tiled": n_tiled, "n_global": n_global}
...
SUPERVISION_HINT = (
    ...
    "  then point the container's PYTHONPATH at $SCRATCH/pyextra (see HOW_TO_RUN_3.md).\n"
...
def aggregate_results(output_dir: Path, experiment_name: str) -> None:
    ...
    for scope, sub in scopes:
        ...
        # how much the global pass contributed, if the columns are present
        if "n_pred_tiled" in sub.columns:
            agg_spec["n_pred_tiled_mean"] = ("n_pred_tiled", "mean")
        if "n_pred_global" in sub.columns:
            agg_spec["n_pred_global_mean"] = ("n_pred_global", "mean")
        ...
        extra = ""
        if "n_pred_global" in sub.columns:
            extra = (f"  preds/run: tiled={sub['n_pred_tiled'].mean():.1f} "
                     f"global={sub['n_pred_global'].mean():.1f}")
...
def main() -> None:
    ...
    global_desc = (f"ON  (downscale={args.global_downscale}, +1 forward pass per anchor)"
                   if args.global_pass and args.global_downscale > 1 else "OFF")

    print("=" * 92)
    print(f" SAM3 TILED MULTI-BOX + GLOBAL PASS BENCHMARK | experiment={exp} | "
    ...
    print(f" global pass    : {global_desc}")
    ...
    if args.shard_index == 0:
        ...
        config["variant"] = "tiled_exemplar_strip_plus_global_pass_fine_grid"
    ...
    if args.dry_run:
        ...
        extra = 1 if (args.global_pass and args.global_downscale > 1) else 0
        print(f"  sampled {sampled} image(s) of this shard -> {total_anchors} anchor runs "
              f"({per_archive})")
        print(f"  tiles per anchor run at 8192x5460: {tiles}"
              f"{f' (+{extra} global pass = {tiles + extra})' if extra else ''}")
        print(f"  => ~{total_anchors * (tiles + extra)} SAM3 forward passes for those "
              f"{sampled} images")
    ...
    global_flag = 1 if (args.global_pass and args.global_downscale > 1) else 0
    ...
    try:
        for img_idx, rec in enumerate(my_records, start=1):
            ...
            # Log-only sanity check, added for the 1536/384 grid: 384 px of overlap is less
            # than the widest annotated plant in this dataset (706 px), so a plant bigger
            # than the overlap can be cut by a tile border in EVERY tile that contains it.
            # Nothing is changed here -- this just tells you how often it can happen, so a
            # low recall on big plants is not a mystery later.
            if n_gt > 0:
                widest = float(max((gt_boxes[:, 2] - gt_boxes[:, 0]).max(),
                                   (gt_boxes[:, 3] - gt_boxes[:, 1]).max()))
                if widest > args.overlap:
                    n_big = int(((gt_boxes[:, 2] - gt_boxes[:, 0] > args.overlap) |
                                 (gt_boxes[:, 3] - gt_boxes[:, 1] > args.overlap)).sum())
                    print(f"      NOTE: {n_big}/{n_gt} GT box(es) in {rec.image_id} are larger "
                          f"than the tile overlap ({args.overlap}px; widest {widest:.0f}px). "
                          f"Those may be cut at every tile border they touch.")
            ...
            for anchor_idx in anchors:
                ...
                # warn ONCE per anchor, not once per tile
                strip_w = strip_content_width(exemplar_crops, args.margin)
                if strip_w > args.tile_size:
                    print(f"      NOTE: exemplar strip ({strip_w}px) is wider than the tile "
                          f"({args.tile_size}px) for {rec.image_id} anchor={anchor_idx}; "
                          f"tiles get downscaled harder than usual.")

                boxes, scores, masks, stats = run_sam3_pipeline(
                ...
                if args.save_detections:
                    det_writer.write({
                        ...
                        "image_height": img_h,
                        "n_tiles": n_tiles,
                        "global_pass": global_flag,
                        "global_downscale": args.global_downscale,
                        "n_pred": len(boxes),
                        "n_pred_tiled": stats["n_tiled"],
                        "n_pred_global": stats["n_global"],
                ...
                if args.save_masks:
                    mask_writer.write({
                        ...
                        # and the factor its resolution must be multiplied by (1.0 for tiled
                        # detections, --global-downscale for the ones from the global pass)
                        "masks": [{"rle": encode_rle(m), "tile_origin": [int(ox), int(oy)],
                                   "scale": float(sc)}
                                  for (m, ox, oy, sc) in masks if m is not None],
                    })
                ...
                print(f"    [{exp}] run #{n_runs} | {rec.image_id} | anchor={anchor_idx} "
                      ...
                      f"preds={len(boxes)} (tiled={stats['n_tiled']}, global={stats['n_global']}) | "
