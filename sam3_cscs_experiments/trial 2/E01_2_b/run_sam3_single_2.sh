...
#  run_sam3_single_2.sh  ==>  dispatcher for the SAM3 TILED SINGLE-BOX + GLOBAL PASS Rumex
#  experiment (E01_2_b)
#
#  Sets up the environment, decides where everything lives under $SCRATCH, and launches
#  infer_sam3_single_2.py -- the port of the E01_2 Colab notebook: tiling ON (2000 px
#  tiles, 800 px overlap), ONE exemplar crop in the strip (the anchor alone), plus one
...
EXPERIMENT_NAME="${EXPERIMENT_NAME:-E01_2_b}"
...
PRESET="${PRESET:-single}"              # single = 1 exemplar crop (E01_2_b), multi = 3
...
try:
    ...
    print("  -> inference will refuse to start. Run './run_sam3_single_2.sh download' on a login node.")
PY
echo "========================"
...
PY
        ...
        echo "      with: ./run_sam3_single_2.sh dryrun   (the PYTHON DEBUG block prints it)."
        ;;

    dryrun)
        # No model, no GPU: just proves the dataset layout is understood and prints the cost.
        python "${SCRIPT_DIR}/infer_sam3_single_2.py" "${COMMON_ARGS[@]}" --dry-run "$@"
        ;;
    ...
    run)
        ...
        if [[ ! -d "${HF_HOME}" || -z "$(ls -A "${HF_HOME}" 2>/dev/null)" ]]; then
            ...
            echo "         Run './run_sam3_single_2.sh download' on a login node first."
        fi
        ...
        for ((i = 0; i < NUM_GPUS; i++)); do
            ...
            python "${SCRIPT_DIR}/infer_sam3_single_2.py" \
                "${COMMON_ARGS[@]}" \
                --num-shards "${NUM_GPUS}" \
                --shard-index "${i}" \
                --no-aggregate \
