...
E01_2_b is the ONE-exemplar variant WITH an extra whole-image context pass: tiling ON
(2000 px tiles, 800 px overlap), the prompt is a SINGLE crop (the anchor box alone), and
on top of the tiled sweep the whole image is also run once at 1/GLOBAL_DOWNSCALE
resolution. The two detection sets are then merged by a single global NMS.
...
For every image, and then for every ground-truth box in that image ("anchor"):
  ...
  2. TILING                   (tile_bboxes)
     ...
     At the defaults (2000 / 800) a 8192x5460 image becomes 28 tiles.
...
CSV_COLUMNS = [
    ...
    "n_pred_tiled",     # raw detections contributed by the tiled sweep   (E01_2_b extra)
    "n_pred_global",    # raw detections contributed by the global pass   (E01_2_b extra)
    "image_width",
    "image_height",
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="SAM3 tiled, single-exemplar Rumex detection benchmark with an extra "
                    "downscaled whole-image pass (inference + evaluation). Port of the "
                    "E01_2 Colab notebook.",
    ...
    g.add_argument("--experiment-name", default="E01_2_b",
    ...
    g.add_argument("--preset", choices=["single", "multi"], default=None,
                   help="Convenience: single = 1 exemplar crop, the anchor alone (E01_2_b, "
                        "the default), multi = 3 exemplar crops (reproduces E02_2_b's prompt "
    ...
    g.add_argument("--tile-size", type=int, default=2000,
                   help="Notebook value. Comfortably larger than the widest annotated plant "
                        "(706 px), leaving ~65%% of the tile as context.")
    g.add_argument("--overlap", type=int, default=800,
                   help="Notebook value. Larger than the widest plant, so every plant is "
                        "fully inside at least one tile.")
    ...
    gp.add_argument("--no-global-pass", dest="global_pass", action="store_false",
                    help="Tiles only. This reproduces the original E01_2 prompt/geometry "
                         "combination and isolates what the global pass contributes.")
    ...
    cache.add_argument("--cache-tiles", dest="cache_tiles", action="store_true", default=True,
                       help="Crop the tiles once per image and reuse them for every anchor of "
                            "that image (the notebook's optimisation; 28 crops instead of "
                            "28 x n_anchors). Costs ~300 MB of RAM at tile-size 2000.")
...
def strip_content_width(crop_images: Sequence[Image.Image], margin: int) -> int:
    ...
    # per anchor instead of once per tile (28 identical warnings help nobody).
...
SUPERVISION_HINT = (
    ...
    "  then point the container's PYTHONPATH at $SCRATCH/pyextra (see HOW_TO_RUN_single_2.md).\n"
...
def main() -> None:
    ...
    if args.shard_index == 0:
        ...
        config["variant"] = "tiled_exemplar_strip_plus_global_pass"
