...
#  submit_sam3_2.sh  --  SLURM batch script for the SAM3 TILED MULTI-BOX + GLOBAL PASS
#  Rumex experiment (E02_2_b: 3 exemplar crops, tiling ON at 2000/800, one extra
#  downscaled whole-image pass, no batching)

#SBATCH --no-requeue
#SBATCH --account="go077"
#SBATCH --job-name="sam3_e02_2b"
#SBATCH --output=sam3_2_%j.out
#SBATCH --error=sam3_2_%j.err
...
# --time is 24h, same as E01_2. The tiles are bigger here (2000 instead of 1000) so an
# 8192x5460 image is only 25 tiles instead of ~70, but each forward pass is heavier and
# the global pass adds a 26th. If it overruns, just resubmit -- the job resumes from the
# shard CSVs.
...
chmod +x "${SCRIPT_DIR}/run_sam3_2.sh"

# Defaults for this job; any of these can be overridden from the submitting shell.
export EXPERIMENT_NAME="${EXPERIMENT_NAME:-E02_2_b}"
...
# run_sam3_2.sh sets. Do NOT rely on the compute node reaching PyPI -- it cannot. If the
# image's transformers is too old for Sam3Model, build a dedicated image
# (HOW_TO_RUN_2.md, "Problem A").
srun \
    --environment=yolo26 \
    --container-workdir="$PWD" \
    --cpu-bind=cores \
    bash -c "${SCRIPT_DIR}/run_sam3_2.sh run"