...
E02_2_b is the three-exemplar variant WITH an extra whole-image context pass: tiling ON
(2000 px tiles, 800 px overlap) and the prompt is THREE crops (the anchor box plus two
other ground-truth boxes from the same image), and on top of the tiled sweep the whole
image is also run once at 1/GLOBAL_DOWNSCALE resolution. The two detection sets are then
merged by a single global NMS.
...
For every image, and then for every ground-truth box in that image ("anchor"):
  ...
  2. TILING                   (tile_bboxes)
     ...
     At the defaults (2000 / 800) a 8192x5460 image becomes 25 tiles.
...
CSV_COLUMNS = [
    ...
    "n_pred_tiled",     # raw detections contributed by the tiled sweep    (E02_2_b extra)
    "n_pred_global",    # raw detections contributed by the global pass    (E02_2_b extra)
    "image_width",
    "image_height",
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    ...
    g.add_argument("--experiment-name", default="E02_2_b",
    ...
    g.add_argument("--preset", choices=["single", "multi"], default=None,
                   help="Convenience: multi = 3 exemplar crops (E02_2_b, the default), "
                        "single = 1 exemplar crop, the anchor alone (reproduces E01_2's "
                        "prompt with this script). --n-exemplars always wins over the preset.")
    ...
    cache.add_argument("--cache-tiles", dest="cache_tiles", action="store_true", default=True,
                       help="Crop the tiles once per image and reuse them for every anchor of "
                            "that image (the notebook's optimisation; ~25 crops instead of "
                            "25 x n_anchors). Costs ~300 MB of RAM at tile-size 2000.")
...
def strip_content_width(crop_images: Sequence[Image.Image], margin: int) -> int:
    ...
    # per anchor instead of once per tile (25 identical warnings help nobody).
...
SUPERVISION_HINT = (
    ...
    "  then point the container's PYTHONPATH at $SCRATCH/pyextra (see HOW_TO_RUN_2.md).\n"
