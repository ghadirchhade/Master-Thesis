#!/bin/bash -l
#
#  E02_2_submit_sam3.sh  --  SLURM batch script for experiment E02_2
#      (SAM3 exemplar-prompted Rumex detection, cluster port of E02_2.ipynb)
#
#  One node, 4 GPUs. PHASE 1 (inference) is split across the 4 GPUs by
#  E02_2_run_sam3.sh; PHASE 2 (the offline evaluation) runs once at the end,
...
#SBATCH --job-name="E02_2_sam3"
#SBATCH --output=E02_2_sam3_%j.out
#SBATCH --error=E02_2_sam3_%j.err
...
chmod +x "${SCRIPT_DIR}/E02_2_run_sam3.sh"

# Defaults for this job; any of these can be overridden from the submitting shell,
# e.g.  DTYPE=float16 sbatch E02_2_submit_sam3.sh
...
# --- notebook CELL 3 configuration (E02_2: 3 exemplars, tiling ON) -----------
...
# E02_2_run_sam3.sh sets. Do NOT rely on the compute node reaching PyPI -- it cannot. If the
# image's transformers is too old for Sam3Model, build a dedicated image
# (E02_2_HOW_TO_RUN.md, "Problem A").
srun \
    --environment=yolo26 \
    --container-workdir="$PWD" \
    --cpu-bind=cores \
    bash -c "${SCRIPT_DIR}/E02_2_run_sam3.sh run"
