...
chmod +x E02_2_run_sam3.sh E02_2_submit_sam3.sh
ls -l
```
...
NUM_GPUS=1 \
./E02_2_run_sam3.sh run --limit-images 2
```
...
SAM3 ready. model device: cuda:0
  [E02_2_smoke] shard0 run #1 | AGS_Multi_Rumex/2022.../DJI_0001 | anchor=0 (1/12) | prompt=0+7+3 | tiles=70 | pre-NMS detections=41 | 58.3s
...
...
tail -f E02_2_sam3_1234567.out                      # everything, live
...
```bash
cat $SCRATCH/experiments/sam3/E02_2/raw_detections/runs_manifest_E02_2_shard*.csv \
  | grep -c E02_2
ls $SCRATCH/experiments/sam3/E02_2/raw_detections/*.npz | wc -l
```
...
```bash
ls -R $SCRATCH/experiments/sam3/E02_2/ | head -40
```
...
| `raw_detections/runs_manifest_E02_2_shard*.csv` | which runs are finished; this is what resume reads |
...
```bash
cd $SCRATCH/experiments/sam3/E02_2/metrics
...
```
Resuming: 4821 run(s) already finished for E02_2; skipped.
```
...
OUTPUT_DIR=$SCRATCH/experiments/sam3/E02_2 \
./E02_2_run_sam3.sh evaluate
```
...
2. **`BATCH_SIZE=8 sbatch E02_2_submit_sam3.sh`** — the notebook used 4 because a T4 has 16 GB;
...
3. **`--max-anchors-per-image 5`** — instead of every ground-truth box taking a turn as the
   ...
   in the thesis. Pass it after the mode: `./E02_2_run_sam3.sh run --max-anchors-per-image 5`.
...
then change the one line at the bottom of `E02_2_submit_sam3.sh`:
...
```bash
THRESHOLD=0.05 EXPERIMENT_NAME=E02_2_lowconf \
OUTPUT_DIR=$SCRATCH/experiments/sam3/E02_2_lowconf sbatch E02_2_submit_sam3.sh
```
...
| Host RAM fills up | `./E02_2_run_sam3.sh run --no-cache-tiles` — tiles are then cropped on demand instead of all being held in RAM (`CACHE_TILES_IN_MEMORY = False` in the notebook) |
...
```bash
DTYPE=float16           sbatch E02_2_submit_sam3.sh   # exactly what the notebook used
BATCH_SIZE=8            sbatch E02_2_submit_sam3.sh   # faster on a big GPU
THRESHOLD=0.05          sbatch E02_2_submit_sam3.sh   # save more detections for the AP curve
BEST_CONFIDENCE=0.30    sbatch E02_2_submit_sam3.sh   # different operating point
BEST_NMS_IOU=0.50       sbatch E02_2_submit_sam3.sh   # different NMS
N_EXEMPLARS=1           sbatch E02_2_submit_sam3.sh   # single visual prompt instead of 3
USE_TILING=0            sbatch E02_2_submit_sam3.sh   # whole image as one tile
NUM_GPUS=2              sbatch E02_2_submit_sam3.sh   # fewer GPUs
DATASET_ROOT=/some/path sbatch E02_2_submit_sam3.sh   # dataset elsewhere
```
