#!/bin/bash
#
# E02_2_run_sam3.sh  ==>  dispatcher for experiment E02_2
...
try:
    ...
    print("  -> inference will refuse to start. Run './E02_2_run_sam3.sh download' on a login node.")
try:
    import pandas
...
PY
        ...
        echo "      with: ./E02_2_run_sam3.sh dryrun   (the PYTHON DEBUG block prints the result)."
        ;;
    ...
    run)
        ...
        if [[ ! -d "${HF_HOME}" || -z "$(ls -A "${HF_HOME}" 2>/dev/null)" ]]; then
            ...
            echo "         Run './E02_2_run_sam3.sh download' on a login node first."
        fi
