#!/bin/bash
#
# E02_1_run_sam3.sh  ==>  dispatcher for experiment E02_1
...
#
# NOTE: E02_1 and E02_2 share $HF_HOME and $PYEXTRA, so if you already ran
#       './E02_2_run_sam3.sh download' you can skip the download mode here.
...
MAX_DIM="${MAX_DIM:-1024}"                       # whole-image resize cap, no tiling

THRESHOLD="${THRESHOLD:-0.30}"                   # SAM3_INFERENCE_THRESHOLD
MASK_THRESHOLD="${MASK_THRESHOLD:-0.40}"         # MASK_THRESHOLD
DTYPE="${DTYPE:-bfloat16}"                       # notebook loaded the model in float32
...
try:
    ...
    print("  -> inference will refuse to start. Run './E02_1_run_sam3.sh download' on a login node.")
try:
    import pandas
...
PY
        ...
        echo "      with: ./E02_1_run_sam3.sh dryrun   (the PYTHON DEBUG block prints the result)."
        ;;
    ...
    run)
        ...
        if [[ ! -d "${HF_HOME}" || -z "$(ls -A "${HF_HOME}" 2>/dev/null)" ]]; then
            ...
            echo "         Run './E02_1_run_sam3.sh download' on a login node first."
        fi
