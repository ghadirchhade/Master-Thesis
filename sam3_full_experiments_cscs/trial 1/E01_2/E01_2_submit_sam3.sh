#!/bin/bash -l
#
#  E01_2_submit_sam3.sh  --  SLURM batch script for experiment E01_2
#      (SAM3 SINGLE-exemplar-prompted Rumex detection, tiling ON,
#       cluster port of E01_2.ipynb)
#
#  Same pipeline as E02_2 with ONE visual prompt instead of three.
#
#  One node, 4 GPUs. PHASE 1 (inference) is split across the 4 GPUs by
#  E01_2_run_sam3.sh; PHASE 2 (the offline evaluation) runs once at the end,
...
#SBATCH --job-name="E01_2_sam3"
#SBATCH --output=E01_2_sam3_%j.out
#SBATCH --error=E01_2_sam3_%j.err
...
chmod +x "${SCRIPT_DIR}/E01_2_run_sam3.sh"

# Defaults for this job; any of these can be overridden from the submitting shell,
# e.g.  DTYPE=float16 sbatch E01_2_submit_sam3.sh
...
# --- notebook CELL 3 configuration (E01_2: 1 exemplar, tiling ON) ------------
...
# E01_2_run_sam3.sh sets. Do NOT rely on the compute node reaching PyPI -- it cannot. If the
# image's transformers is too old for Sam3Model, build a dedicated image
# (E01_2_HOW_TO_RUN.md, "Problem A").
srun \
    --environment=yolo26 \
    --container-workdir="$PWD" \
    --cpu-bind=cores \
    bash -c "${SCRIPT_DIR}/E01_2_run_sam3.sh run"
