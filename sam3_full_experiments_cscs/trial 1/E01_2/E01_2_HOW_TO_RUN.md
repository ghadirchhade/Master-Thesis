...
What E01_2 is: SAM3 prompted with **1 exemplar crop**, tiling ON (1000 px tiles, 150 px
overlap), run over both `AGS_Multi_Rumex` and `AgsSpringRumex` pooled as one dataset.

**E01_2 vs E02_2 in one line:** they are the *same* pipeline; E01_2 shows SAM3 one example
plant per run (the anchor itself), E02_2 shows three. The notebooks differ in exactly two
configuration lines — `EXPERIMENT_NAME` and `N_EXEMPLARS` — so the pair answers one question
cleanly: **does adding more visual prompts help?**

This is the cluster port of `E01_2.ipynb`. Like the notebook, it is a **two-phase** pipeline:
...
chmod +x E01_2_run_sam3.sh E01_2_submit_sam3.sh
ls -l
```
...
> **If you already ran `./E02_2_run_sam3.sh download` (or the E02_1 one), skip this step.**
...
It writes to a throwaway folder so it cannot pollute the real E01_2 results.

```bash
EXPERIMENT_NAME=E01_2_smoke \
OUTPUT_DIR=$SCRATCH/experiments/sam3/_smoke_E01_2 \
NUM_GPUS=1 \
./E01_2_run_sam3.sh run --limit-images 2
```
...
SAM3 ready. model device: cuda:0
  [E01_2_smoke] shard0 run #1 | AGS_Multi_Rumex/2022.../DJI_0001 | anchor=0 (1/12) | prompt=0 | tiles=70 | pre-NMS detections=41 | 58.3s
...
...
```bash
cd $HOME/2025-OverneyTechnologiesProject/7_cscs_experiments/sam3
sbatch E01_2_submit_sam3.sh
```

**You should see:** `Submitted batch job 1234567`

That is it. No arguments needed — E01_2 is the default. The job asks for 1 node, 4 GPUs,
...
tail -f E01_2_sam3_1234567.out                      # everything, live
...
```bash
cat $SCRATCH/experiments/sam3/E01_2/raw_detections/runs_manifest_E01_2_shard*.csv \
  | grep -c E01_2
ls $SCRATCH/experiments/sam3/E01_2/raw_detections/*.npz | wc -l
```
...
```bash
ls -R $SCRATCH/experiments/sam3/E01_2/ | head -40
```
...
| `raw_detections/runs_manifest_E01_2_shard*.csv` | which runs are finished; this is what resume reads |
...
```bash
cd $SCRATCH/experiments/sam3/E01_2/metrics
...
```
Resuming: 4821 run(s) already finished for E01_2; skipped.
```
...
OUTPUT_DIR=$SCRATCH/experiments/sam3/E01_2 \
./E01_2_run_sam3.sh evaluate
```
...
2. **`BATCH_SIZE=8 sbatch E01_2_submit_sam3.sh`** — the notebook used 4 because a T4 has 16 GB;
...
3. **`--max-anchors-per-image 5`** — instead of every ground-truth box taking a turn as the
   ...
   in the thesis. Pass it after the mode: `./E01_2_run_sam3.sh run --max-anchors-per-image 5`.
4. **More GPUs** — needs a job array; ask and I will write one.

## Comparing E01_2 against E02_2

The two experiments write to different folders (`$SCRATCH/experiments/sam3/E01_2` and
`.../E02_2`), use the same frozen operating point, the same eval IoU and the same metric
columns, so their headline tables stack directly:

```bash
head -1 $SCRATCH/experiments/sam3/E02_2/metrics/experiment_summary.csv
tail -n +2 -q $SCRATCH/experiments/sam3/E0{1,2}_2/metrics/experiment_summary.csv
```

The `n_exemplars` column (1 vs 3) is the only input that differs, which is exactly what makes
the difference in `F1_mean` / `AP50_mean` attributable to the prompt count.

Two things to say about this in the thesis:

- In `all_gt` mode the comparison is slightly unfair to E02_2 *by construction*: E02_2 removes
  three plants' worth of "free" detections from the pool in `held_out`, E01_2 only one. Quote
  **`held_out`** when you want the honest "does more prompting help" answer, and mention both.
- Runtime is essentially the same. The prompt count changes the exemplar strip, not the tile
  count, so both experiments run ~70 forward passes per anchor.

If you also ran E02_1 (no tiling, 3 prompts), the three together separate the two variables:
E01_2 vs E02_2 isolates the prompt count, and E02_1 vs E02_2 isolates the tiling.
...
then change the one line at the bottom of `E01_2_submit_sam3.sh`:
...
```bash
THRESHOLD=0.05 EXPERIMENT_NAME=E01_2_lowconf \
OUTPUT_DIR=$SCRATCH/experiments/sam3/E01_2_lowconf sbatch E01_2_submit_sam3.sh
```
...
| Host RAM fills up | `./E01_2_run_sam3.sh run --no-cache-tiles` — tiles are then cropped on demand instead of all being held in RAM (`CACHE_TILES_IN_MEMORY = False` in the notebook) |
...
```bash
DTYPE=float16           sbatch E01_2_submit_sam3.sh   # exactly what the notebook used
BATCH_SIZE=8            sbatch E01_2_submit_sam3.sh   # faster on a big GPU
THRESHOLD=0.05          sbatch E01_2_submit_sam3.sh   # save more detections for the AP curve
BEST_CONFIDENCE=0.30    sbatch E01_2_submit_sam3.sh   # different operating point
BEST_NMS_IOU=0.50       sbatch E01_2_submit_sam3.sh   # different NMS
N_EXEMPLARS=3           sbatch E01_2_submit_sam3.sh   # three visual prompts (= E02_2's scheme)
USE_TILING=0            sbatch E01_2_submit_sam3.sh   # whole image as one tile
NUM_GPUS=2              sbatch E01_2_submit_sam3.sh   # fewer GPUs
DATASET_ROOT=/some/path sbatch E01_2_submit_sam3.sh   # dataset elsewhere
```
