#!/bin/bash -l
#
#  E01_2c_submit_sam3.sh  --  SLURM batch script for experiment E01_2c
...
#
#  Single-exemplar sibling of the 3-prompt run: N_EXEMPLARS=1 instead of 3.
#
#  One node, 4 GPUs. PHASE 1 (inference) is split across the 4 GPUs by
#  E01_2c_run_sam3.sh; PHASE 2 (the offline evaluation) runs once at the end,
...
#SBATCH --job-name="E01_2c_sam3"
#SBATCH --output=E01_2c_sam3_%j.out
#SBATCH --error=E01_2c_sam3_%j.err
...
chmod +x "${SCRIPT_DIR}/E01_2c_run_sam3.sh"

# Defaults for this job; any of these can be overridden from the submitting shell,
# e.g.  DTYPE=float16 sbatch E01_2c_submit_sam3.sh
export EXPERIMENT_NAME="${EXPERIMENT_NAME:-E01_2c}"
...
# --- notebook CELL 3 configuration (E01_2c: 1 exemplar, 1536/384 tiles + global pass) ---
export N_EXEMPLARS="${N_EXEMPLARS:-1}"
export USE_TILING="${USE_TILING:-1}"
export TILE_SIZE="${TILE_SIZE:-1536}"             # E01_2c tile size
export OVERLAP="${OVERLAP:-384}"                  # E01_2c: 25 percent overlap
...
# E01_2c_run_sam3.sh sets. Do NOT rely on the compute node reaching PyPI -- it cannot. If the
# image's transformers is too old for Sam3Model, build a dedicated image
# (E01_2c_HOW_TO_RUN.md, "Problem A").
srun \
    --environment=yolo26 \
    --container-workdir="$PWD" \
    --cpu-bind=cores \
    bash -c "${SCRIPT_DIR}/E01_2c_run_sam3.sh run"
