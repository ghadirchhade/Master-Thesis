#!/usr/bin/env python3
# =============================================================================
#  E01_2c_infer_sam3.py
# =============================================================================
#  Cluster (CSCS) port of the Colab notebook  E01_2c.ipynb
#
#  E01_2c is the SINGLE-EXEMPLAR sibling of E02_2c: N_EXEMPLARS = 1, so each run
#  shows SAM3 exactly ONE example plant (the anchor itself) instead of three.
#  The tiling and the global pass are identical to E02_2c, so the pair isolates
#  the prompt count at this tile size.
#
#  Like E02_2c, it is the E02_2 pipeline (same strip composition, same offline
#  NMS, same all_gt / held_out evaluation) with THREE changes of its own:
#    1. BIGGER TILES: TILE_SIZE = 1536, OVERLAP = 384 (a 25% overlap).
#       The E02_2 reference used 1000 / 150 and the E01_2b sibling uses
#       2000 / 800, so E01_2c sits between them: 35 tiles per 8192x5460 image,
#       against 70 and 28 respectively.
...
SUPERVISION_HINT = (
    ...
    "have no internet: run './E01_2c_run_sam3.sh download' on a LOGIN node first, "
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="E01_2c - SAM3 single-exemplar-prompted Rumex detection with 1536/384 "
                    "tiles and an optional global-context pass, cluster port of E01_2c.ipynb.",
    ...
    g.add_argument("--experiment-name", default="E01_2c",
    ...
    g.add_argument("--n-exemplars", type=int, default=1,
                   help="N_EXEMPLARS. E01_2c uses 1 (a single visual prompt: the "
                        "anchor itself). Set 3 to reproduce E02_2c's prompt scheme.")
    ...
    g.add_argument("--tile-size", type=int, default=1536,
                   help="TILE_SIZE. E01_2c uses 1536, between the E02_2 reference's "
                        "1000 and the E01_2b sibling's 2000. Still comfortably larger "
                        "than the biggest measured plant.")
    g.add_argument("--overlap", type=int, default=384,
                   help="OVERLAP. E01_2c uses 384 = 25%% of the tile (the E02_2 "
                        "reference used 150, E01_2b uses 800). The notebook flags that "
                        "this should exceed the widest plant in the dataset -- worth "
                        "checking against the measured plant-width stats.")
...
def compose_tile_with_exemplars(tile_img: Image.Image, crop_images: Sequence[Image.Image],
    ...
    Input : tile_img    - PIL image of the tile
            crop_images - list of PIL exemplar crops (1 for E01_2c, 3 for E02_2c)
...
def run_inference(args, paths: Paths, records: List[ImageRecord]) -> None:
    ...
    # 1536px tiles plus the global pass use far more host RAM than
...
def main() -> None:
    ...
    if args.evaluate_only:
        ...
        if not _check_pandas():
            print("\nERROR: pandas is required for the evaluation. See "
                  "'Problem B' in E01_2c_HOW_TO_RUN.md.")
            sys.exit(2)
    ...
    print("=" * 92)
    print(f" E01_2c SAM3 SINGLE-EXEMPLAR PIPELINE | experiment={exp} | "
