# HOW TO RUN — Experiment E01_2c

What E01_2c is: SAM3 prompted with **1 exemplar crop**, tiling ON with **1536 px tiles and a
...
**E01_2c is the single-exemplar sibling of E02_2c.** The two notebooks differ only in
`EXPERIMENT_NAME` and `N_EXEMPLARS` (1 here, 3 there) — the tiling, the global pass and every
evaluation step are the same, so that pair isolates the prompt count at this tile size.

**E01_2c vs the other single-exemplar runs** — same prompts, same strip composition, same
filters, same frozen operating point, same evaluation. Only the tiling geometry differs:
...
E01_2c is a **middle tiling point** between the 1000/150 reference geometry and the 2000/800
sibling. The global pass adds whole-field context that a tile cannot see. Both are
inference-time only — every evaluation cell downstream is untouched, which is what keeps E01_2c
comparable to the others.

> **Check the overlap against your plant widths.** The notebook's own CELL 3 comment flags this:
> `OVERLAP` is set to 25 % of the tile (384 px) rather than derived from the data, and it notes
> that the overlap *should* exceed the widest plant so no plant is split across tiles. 384 px is
> well above the reference run's 150 px, but if your measured max plant width is larger than
> 384 px, raise `OVERLAP` and give that run its own `EXPERIMENT_NAME`.

This is the cluster port of `E01_2c.ipynb`. Like the notebook, it is a **two-phase** pipeline:
...
chmod +x E01_2c_run_sam3.sh E01_2c_submit_sam3.sh
ls -l
```
...
run, so E01_2c is **not** 2× faster than E01_2 just because it has half the tiles. Use the
...
It writes to a throwaway folder so it cannot pollute the real E01_2c results.

```bash
EXPERIMENT_NAME=E01_2c_smoke \
OUTPUT_DIR=$SCRATCH/experiments/sam3/_smoke_E01_2c \
NUM_GPUS=1 \
./E01_2c_run_sam3.sh run --limit-images 2
```
...
SAM3 ready. model device: cuda:0
  [E01_2c_smoke] shard0 run #1 | AGS_Multi_Rumex/2022.../DJI_0001 | anchor=0 (1/12) | prompt=0 | tiles=35 | global_pass=True | pre-NMS detections=41 | 51.4s
...
...
```bash
cd $HOME/2025-OverneyTechnologiesProject/7_cscs_experiments/sam3
sbatch E01_2c_submit_sam3.sh
```

**You should see:** `Submitted batch job 1234567`

That is it. No arguments needed — E01_2c is the default. The job asks for 1 node, 4 GPUs,
...
tail -f E01_2c_sam3_1234567.out                      # everything, live
tail -f $SCRATCH/experiments/sam3/E01_2c/shard0.log  # just GPU 0
```

Count how many runs are finished so far:

```bash
cat $SCRATCH/experiments/sam3/E01_2c/raw_detections/runs_manifest_E01_2c_shard*.csv \
  | grep -c E01_2c
ls $SCRATCH/experiments/sam3/E01_2c/raw_detections/*.npz | wc -l
```
...
Everything lands in `$SCRATCH/experiments/sam3/E01_2c/`:

```bash
ls -R $SCRATCH/experiments/sam3/E01_2c/ | head -40
```
...
| `raw_detections/runs_manifest_E01_2c_shard*.csv` | which runs are finished; this is what resume reads |
| `run_config_E01_2c.json` | every parameter used, for the thesis appendix |
| `shard0..3.log` | per-GPU logs |

Look at the headline table:

```bash
cd $SCRATCH/experiments/sam3/E01_2c/metrics
...
```
Resuming: 4821 run(s) already finished for E01_2c; skipped.
```
...
OUTPUT_DIR=$SCRATCH/experiments/sam3/E01_2c \
./E01_2c_run_sam3.sh evaluate
```
...
2. **`BATCH_SIZE=8 sbatch E01_2c_submit_sam3.sh`** — the notebook used 4 because a T4 has 16 GB;
...
3. **`--max-anchors-per-image 5`** — instead of every ground-truth box taking a turn as the
   ...
   in the thesis. Pass it after the mode: `./E01_2c_run_sam3.sh run --max-anchors-per-image 5`.
...
```bash
ADD_GLOBAL_CONTEXT_PASS=0 EXPERIMENT_NAME=E01_2c_notglobal \
OUTPUT_DIR=$SCRATCH/experiments/sam3/E01_2c_notglobal sbatch E01_2c_submit_sam3.sh
```
...
```bash
MEM_STOP_THRESHOLD_PCT=85    sbatch E01_2c_submit_sam3.sh   # allow more RAM
ADD_GLOBAL_CONTEXT_PASS=0    sbatch E01_2c_submit_sam3.sh   # drop the biggest allocation
NUM_GPUS=2                   sbatch E01_2c_submit_sam3.sh   # 2 shards instead of 4
MEM_STOP_THRESHOLD_PCT=0     sbatch E01_2c_submit_sam3.sh   # disable (risks an OOM kill)
```

Without `psutil` the guard is simply off and the run behaves like E02_2 in that respect.

## Comparing E01_2c against its siblings

Every experiment writes to its own folder and shares every evaluation setting, so the headline
tables stack directly:

```bash
head -1 $SCRATCH/experiments/sam3/E01_2c/metrics/experiment_summary.csv
for e in E01_2 E01_2b E01_2c E02_2 E02_2b E02_2c; do
...
| Compare | Isolates |
|---|---|
| **E01_2c vs E02_2c** | the prompt count (1 vs 3) at 1536/384 |
| **E01_2 vs E01_2c vs E01_2b** | tile size (1000 / 1536 / 2000) at 1 prompt |
| **E02_2 vs E02_2c vs E02_2b** | tile size at 3 prompts |

Two cautions when writing this up:

- The b and c runs **also** add the global-context pass, which the plain E01_2 / E02_2 runs do
  not have, so any comparison crossing that boundary moves two variables at once. For a clean
  tile-size-only curve, run the `ADD_GLOBAL_CONTEXT_PASS=0` variant described above for the b
  and c runs, or state the caveat explicitly.
- The overlaps are not proportional across the three tile sizes (15 %, 25 %, 40 % of the tile),
  so quote the overlap alongside each tile size rather than presenting it as tile size alone.
...
then change the one line at the bottom of `E01_2c_submit_sam3.sh`:
...
```bash
THRESHOLD=0.05 EXPERIMENT_NAME=E01_2c_lowconf \
OUTPUT_DIR=$SCRATCH/experiments/sam3/E01_2c_lowconf sbatch E01_2c_submit_sam3.sh
```
...
| Host RAM fills up | `./E01_2c_run_sam3.sh run --no-cache-tiles` — tiles are then cropped on demand instead of all being held in RAM (`CACHE_TILES_IN_MEMORY = False` in the notebook) |
...
```bash
DTYPE=float16           sbatch E01_2c_submit_sam3.sh   # exactly what the notebook used
BATCH_SIZE=8            sbatch E01_2c_submit_sam3.sh   # faster on a big GPU
THRESHOLD=0.05          sbatch E01_2c_submit_sam3.sh   # save more detections for the AP curve
BEST_CONFIDENCE=0.30    sbatch E01_2c_submit_sam3.sh   # different operating point
BEST_NMS_IOU=0.50       sbatch E01_2c_submit_sam3.sh   # different NMS
N_EXEMPLARS=3           sbatch E01_2c_submit_sam3.sh   # three prompts (= E02_2c's scheme)
USE_TILING=0            sbatch E01_2c_submit_sam3.sh   # whole image as one tile
ADD_GLOBAL_CONTEXT_PASS=0 sbatch E01_2c_submit_sam3.sh # tiles only, no whole-image pass
GLOBAL_DOWNSCALE=4      sbatch E01_2c_submit_sam3.sh   # cheaper/coarser global pass
TILE_SIZE=1024          sbatch E01_2c_submit_sam3.sh   # smaller tiles if you hit OOM
OVERLAP=600             sbatch E01_2c_submit_sam3.sh   # if your widest plant exceeds 384 px
MEM_STOP_THRESHOLD_PCT=85 sbatch E01_2c_submit_sam3.sh # allow more host RAM
NUM_GPUS=2              sbatch E01_2c_submit_sam3.sh   # fewer GPUs
DATASET_ROOT=/some/path sbatch E01_2c_submit_sam3.sh   # dataset elsewhere
```
