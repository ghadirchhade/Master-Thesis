#!/bin/bash
#
# E01_2c_run_sam3.sh  ==>  dispatcher for experiment E01_2c
#     (SAM3 SINGLE-exemplar-prompted Rumex detection with 1536/384 TILES and an
#      optional GLOBAL-CONTEXT PASS, cluster port of E01_2c.ipynb)
#
# E01_2c is the single-exemplar sibling of E02_2c (N_EXEMPLARS=1 instead of 3);
# the tiling and the global pass are identical. Both differ from the E02_2
# reference in three ways: bigger tiles (1536 px with a 25 percent overlap, i.e.
# between the 1000/150 reference and the 2000/800 sibling), the extra whole-image
# pass pooled into the same pre-NMS detection set, and a host-RAM guard that
# stops a shard cleanly rather than being killed.
#
# Sets up the environment, decides where everything lives under $SCRATCH and
# launches E01_2c_infer_sam3.py.
#
# MODES
...
N_EXEMPLARS="${N_EXEMPLARS:-1}"                  # E01_2c: ONE visual prompt (the anchor)
USE_TILING="${USE_TILING:-1}"                    # 1 = overlapping tiles, 0 = whole image
TILE_SIZE="${TILE_SIZE:-1536}"                   # E01_2c tile size
OVERLAP="${OVERLAP:-384}"                        # E01_2c: 25 percent of the tile
...
# ------------------ optional global-context pass (CELL 16) -------------------
ADD_GLOBAL_CONTEXT_PASS="${ADD_GLOBAL_CONTEXT_PASS:-1}"   # 1 = on (notebook default)
GLOBAL_DOWNSCALE="${GLOBAL_DOWNSCALE:-2}"                 # shrink factor for that pass

# ------------------------------ safety net -----------------------------------
# Stop a shard CLEANLY when host RAM exceeds this percentage, instead of being
# OOM-killed. Finished runs stay on disk; resubmit to resume. 0 disables it.
MEM_STOP_THRESHOLD_PCT="${MEM_STOP_THRESHOLD_PCT:-70}"
...
try:
    ...
    print("  -> inference will refuse to start. Run './E01_2c_run_sam3.sh download' on a login node.")
try:
    import pandas
...
try:
    ...
    print("psutil:", psutil.__version__,
          "| host RAM now:", f"{psutil.virtual_memory().percent:.0f}%",
          "-> RAM guard: ENABLED")
except Exception as exc:
    print("psutil import FAILED:", exc)
    print("  -> the RAM guard is disabled; the run behaves like the E02_2 reference there.")
PY
echo "========================"
...
# recorded in experiment_summary.csv so the run stays self-documenting
if [[ "${ADD_GLOBAL_CONTEXT_PASS}" == "1" ]]; then
...
PY
        ...
        pip download --no-deps -d "${WHEELS}" \
            pandas matplotlib supervision psutil \
        ...
        echo "      with: ./E01_2c_run_sam3.sh dryrun   (the PYTHON DEBUG block prints the result)."
        ;;
    ...
    run)
        ...
        if [[ ! -d "${HF_HOME}" || -z "$(ls -A "${HF_HOME}" 2>/dev/null)" ]]; then
            ...
            echo "         Run './E01_2c_run_sam3.sh download' on a login node first."
        fi
