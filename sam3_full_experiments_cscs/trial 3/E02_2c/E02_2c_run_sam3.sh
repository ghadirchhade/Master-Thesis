#!/bin/bash
#
# E02_2c_run_sam3.sh  ==>  dispatcher for experiment E02_2c
...
#
# E02_2c differs from the E02_2 reference in three ways: bigger tiles (1536 px
# with a 25 percent overlap, i.e. between the 1000/150 reference and the 2000/800
# sibling), the extra whole-image pass pooled into the same pre-NMS detection
# set, and a host-RAM guard that stops a shard cleanly rather than being killed.
#
# Sets up the environment, decides where everything lives under $SCRATCH and
# launches E02_2c_infer_sam3.py.
#
# MODES
...
TILE_SIZE="${TILE_SIZE:-1536}"                   # E02_2c tile size
OVERLAP="${OVERLAP:-384}"                        # E02_2c: 25 percent of the tile
...
try:
    ...
    print("  -> inference will refuse to start. Run './E02_2c_run_sam3.sh download' on a login node.")
try:
    import pandas
...
PY
        ...
        echo "      with: ./E02_2c_run_sam3.sh dryrun   (the PYTHON DEBUG block prints the result)."
        ;;
    ...
    run)
        ...
        if [[ ! -d "${HF_HOME}" || -z "$(ls -A "${HF_HOME}" 2>/dev/null)" ]]; then
            ...
            echo "         Run './E02_2c_run_sam3.sh download' on a login node first."
        fi
