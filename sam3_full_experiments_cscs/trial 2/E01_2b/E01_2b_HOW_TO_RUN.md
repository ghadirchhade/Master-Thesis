# HOW TO RUN — Experiment E01_2b

What E01_2b is: SAM3 prompted with **1 exemplar crop**, tiling ON with **big tiles**
(2000 px tiles, 800 px overlap), **plus an extra whole-image "global context" pass**, run over
both `AGS_Multi_Rumex` and `AgsSpringRumex` pooled as one dataset.

**E01_2b is the single-exemplar sibling of E02_2b.** The two notebooks differ in exactly two
configuration lines — `EXPERIMENT_NAME` and `N_EXEMPLARS` (1 here, 3 there) — so that pair
isolates the prompt count under the big-tile + global-pass setup.

**E01_2b vs the E02_2 reference** — same strip composition, same filters, same frozen operating
point, same evaluation. Three things differ (four counting the prompt count):

| | E02_2 reference | E01_2b |
|---|---|---|
| tile / overlap | 1000 / 150 | **2000 / 800** |
| tiles per 8192×5460 image | 70 | **28** |
| global-context pass | none | **1 extra whole-image pass per anchor**, downscaled ×2, pooled into the same pre-NMS detections (`tile_id = -1`) |
| host-RAM guard | none | **stops the shard cleanly** instead of being OOM-killed |

The bigger tile comfortably exceeds the largest measured plant, and the 800 px overlap exceeds
the max plant width, so every plant is captured whole in at least one tile. The global pass adds
whole-field context that a tile cannot see. Both changes are inference-time only — every
evaluation cell downstream is untouched, which is what keeps E01_2b comparable to the others.

This is the cluster port of `E01_2b.ipynb`. Like the notebook, it is a **two-phase** pipeline:
...
chmod +x E01_2b_run_sam3.sh E01_2b_submit_sam3.sh
ls -l
```
...
```
  ...
  tiles per anchor run at 8192x5460: 28 (tile=2000, overlap=800)
...
**28 tiles, not 70** — that is the big-tile change. But each tile covers 4× the area of an
E02_2 tile, and the global pass at 4096×2730 is by far the largest single forward pass of the
run, so E01_2b is **not** 2.5× faster than E02_2 just because it has fewer tiles. Use the smoke
test in Step 7 for the real per-run timing.
...
It writes to a throwaway folder so it cannot pollute the real E01_2b results.

```bash
EXPERIMENT_NAME=E01_2b_smoke \
OUTPUT_DIR=$SCRATCH/experiments/sam3/_smoke_E01_2b \
NUM_GPUS=1 \
./E01_2b_run_sam3.sh run --limit-images 2
```
...
SAM3 ready. model device: cuda:0
  [E01_2b_smoke] shard0 run #1 | AGS_Multi_Rumex/2022.../DJI_0001 | anchor=0 (1/12) | prompt=0 | tiles=28 | global_pass=True | pre-NMS detections=41 | 49.2s
...
...
```bash
cd $HOME/2025-OverneyTechnologiesProject/7_cscs_experiments/sam3
sbatch E01_2b_submit_sam3.sh
```

**You should see:** `Submitted batch job 1234567`

That is it. No arguments needed — E01_2b is the default. The job asks for 1 node, 4 GPUs,
...
tail -f E01_2b_sam3_1234567.out                      # everything, live
tail -f $SCRATCH/experiments/sam3/E01_2b/shard0.log  # just GPU 0
```

Count how many runs are finished so far:

```bash
cat $SCRATCH/experiments/sam3/E01_2b/raw_detections/runs_manifest_E01_2b_shard*.csv \
  | grep -c E01_2b
ls $SCRATCH/experiments/sam3/E01_2b/raw_detections/*.npz | wc -l
```
...
Everything lands in `$SCRATCH/experiments/sam3/E01_2b/`:

```bash
ls -R $SCRATCH/experiments/sam3/E01_2b/ | head -40
```
...
| `raw_detections/runs_manifest_E01_2b_shard*.csv` | which runs are finished; this is what resume reads |
| `run_config_E01_2b.json` | every parameter used, for the thesis appendix |
| `shard0..3.log` | per-GPU logs |

Look at the headline table:

```bash
cd $SCRATCH/experiments/sam3/E01_2b/metrics
...
```
Resuming: 4821 run(s) already finished for E01_2b; skipped.
```
...
OUTPUT_DIR=$SCRATCH/experiments/sam3/E01_2b \
./E01_2b_run_sam3.sh evaluate
```
...
2. **`BATCH_SIZE=8 sbatch E01_2b_submit_sam3.sh`** — the notebook used 4 because a T4 has 16 GB;
...
3. **`--max-anchors-per-image 5`** — instead of every ground-truth box taking a turn as the
   ...
   in the thesis. Pass it after the mode: `./E01_2b_run_sam3.sh run --max-anchors-per-image 5`.
4. **More GPUs** — needs a job array; ask and I will write one.

## The two big-tile-variant features
...
```bash
ADD_GLOBAL_CONTEXT_PASS=0 EXPERIMENT_NAME=E01_2b_notglobal \
OUTPUT_DIR=$SCRATCH/experiments/sam3/E01_2b_notglobal sbatch E01_2b_submit_sam3.sh
```
...
2000 px tiles plus a 4096×2730 global pass use far more host RAM than E02_2's 1000 px tiles,
...
```bash
MEM_STOP_THRESHOLD_PCT=85    sbatch E01_2b_submit_sam3.sh   # allow more RAM
ADD_GLOBAL_CONTEXT_PASS=0    sbatch E01_2b_submit_sam3.sh   # drop the biggest allocation
NUM_GPUS=2                   sbatch E01_2b_submit_sam3.sh   # 2 shards instead of 4
MEM_STOP_THRESHOLD_PCT=0     sbatch E01_2b_submit_sam3.sh   # disable (risks an OOM kill)
```

Without `psutil` the guard is simply off and the run behaves like E02_2 in that respect.

## Comparing E01_2b against its siblings

Every experiment writes to its own folder and shares every evaluation setting, so the headline
tables stack directly:

```bash
head -1 $SCRATCH/experiments/sam3/E01_2b/metrics/experiment_summary.csv
for e in E01_2 E02_2 E01_2b E02_2b; do
...
Which pair answers which question:

| Compare | Isolates |
|---|---|
| **E01_2b vs E02_2b** | the prompt count (1 vs 3) under big tiles + global pass |
| **E01_2 vs E02_2** | the prompt count under the original 1000/150 tiles |
| **E01_2b vs E01_2** | big tiles + global pass, at 1 prompt |
| **E02_2b vs E02_2** | big tiles + global pass, at 3 prompts |

Note that the last two change **two** things at once (tile size *and* the global pass). To
separate those, run the `ADD_GLOBAL_CONTEXT_PASS=0` variant described above.
...
then change the one line at the bottom of `E01_2b_submit_sam3.sh`:
...
```bash
THRESHOLD=0.05 EXPERIMENT_NAME=E01_2b_lowconf \
OUTPUT_DIR=$SCRATCH/experiments/sam3/E01_2b_lowconf sbatch E01_2b_submit_sam3.sh
```
...
| Host RAM fills up | `./E01_2b_run_sam3.sh run --no-cache-tiles` — tiles are then cropped on demand instead of all being held in RAM (`CACHE_TILES_IN_MEMORY = False` in the notebook) |
...
```bash
DTYPE=float16           sbatch E01_2b_submit_sam3.sh   # exactly what the notebook used
BATCH_SIZE=8            sbatch E01_2b_submit_sam3.sh   # faster on a big GPU
THRESHOLD=0.05          sbatch E01_2b_submit_sam3.sh   # save more detections for the AP curve
BEST_CONFIDENCE=0.30    sbatch E01_2b_submit_sam3.sh   # different operating point
BEST_NMS_IOU=0.50       sbatch E01_2b_submit_sam3.sh   # different NMS
N_EXEMPLARS=3           sbatch E01_2b_submit_sam3.sh   # three prompts (= E02_2b's scheme)
USE_TILING=0            sbatch E01_2b_submit_sam3.sh   # whole image as one tile
ADD_GLOBAL_CONTEXT_PASS=0 sbatch E01_2b_submit_sam3.sh # tiles only, no whole-image pass
GLOBAL_DOWNSCALE=4      sbatch E01_2b_submit_sam3.sh   # cheaper/coarser global pass
TILE_SIZE=1500          sbatch E01_2b_submit_sam3.sh   # smaller tiles if you hit OOM
MEM_STOP_THRESHOLD_PCT=85 sbatch E01_2b_submit_sam3.sh # allow more host RAM
NUM_GPUS=2              sbatch E01_2b_submit_sam3.sh   # fewer GPUs
DATASET_ROOT=/some/path sbatch E01_2b_submit_sam3.sh   # dataset elsewhere
```
