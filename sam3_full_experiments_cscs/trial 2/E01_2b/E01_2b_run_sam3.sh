#!/bin/bash
#
# E01_2b_run_sam3.sh  ==>  dispatcher for experiment E01_2b
#     (SAM3 SINGLE-exemplar-prompted Rumex detection with BIG TILES (2000/800)
#      and an optional GLOBAL-CONTEXT PASS, cluster port of E01_2b.ipynb)
#
# E01_2b is the single-exemplar sibling of E02_2b (N_EXEMPLARS=1 instead of 3);
# everything else is identical. Both differ from the E02_2 reference in three
# ways: bigger tiles, the extra whole-image pass pooled into the same pre-NMS
# detection set, and a host-RAM guard that stops a shard cleanly.
#
# Sets up the environment, decides where everything lives under $SCRATCH and
# launches E01_2b_infer_sam3.py.
#
# MODES
...
N_EXEMPLARS="${N_EXEMPLARS:-1}"                  # E01_2b: ONE visual prompt (the anchor)
USE_TILING="${USE_TILING:-1}"                    # 1 = overlapping tiles, 0 = whole image
TILE_SIZE="${TILE_SIZE:-2000}"                   # bigger than the E02_2 reference 1000
OVERLAP="${OVERLAP:-800}"                        # bigger than the E02_2 reference 150
...
try:
    ...
    print("  -> inference will refuse to start. Run './E01_2b_run_sam3.sh download' on a login node.")
try:
    import pandas
...
PY
        ...
        echo "      with: ./E01_2b_run_sam3.sh dryrun   (the PYTHON DEBUG block prints the result)."
        ;;
    ...
    run)
        ...
        if [[ ! -d "${HF_HOME}" || -z "$(ls -A "${HF_HOME}" 2>/dev/null)" ]]; then
            ...
            echo "         Run './E01_2b_run_sam3.sh download' on a login node first."
        fi
