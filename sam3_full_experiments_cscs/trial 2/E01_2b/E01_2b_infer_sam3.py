#!/usr/bin/env python3
# =============================================================================
#  E01_2b_infer_sam3.py
# =============================================================================
#  Cluster (CSCS) port of the Colab notebook  E01_2b.ipynb
#
#  E01_2b is the SINGLE-EXEMPLAR sibling of E02_2b: N_EXEMPLARS = 1, so each run
#  shows SAM3 exactly ONE example plant (the anchor itself) instead of three.
#  Everything else is identical to E02_2b, and the two notebooks differ in
#  exactly those two configuration lines (EXPERIMENT_NAME and N_EXEMPLARS).
#
#  Like E02_2b, it is the E02_2 pipeline (same strip composition, same offline
#  NMS, same all_gt / held_out evaluation) with THREE changes of its own:
#    1. BIGGER TILES: TILE_SIZE = 2000, OVERLAP = 800 (E02_2 used 1000 / 150).
#       The tile comfortably exceeds the largest measured plant, and the overlap
#       exceeds the max plant width, so a plant is captured whole in at least
#       one tile. 28 tiles per 8192x5460 image instead of E02_2's 70.
#    2. OPTIONAL GLOBAL-CONTEXT PASS (on by default): one EXTRA pass over the
#       whole image downscaled by GLOBAL_DOWNSCALE, run through the SAME
#       exemplar-strip + SAM3 + filter pipeline as a tile. Its detections are
#       rescaled to full resolution and pooled with the tiled detections BEFORE
#       NMS, tagged tile_id = -1, so nothing downstream needs special-casing.
#    3. A HOST-RAM GUARD (MEM_STOP_THRESHOLD_PCT): bigger tiles plus the global
#       pass use far more host RAM, so the loop stops CLEANLY instead of being
#       OOM-killed, and the next submission resumes from the manifests.
#
...
#            compose  exemplar strip + tile          (CELL 12 + 13)
#            run SAM3 once at SAM3_INFERENCE_THRESHOLD = 0.30 (CELL 15)
#            filter to the tile region + plausibility filter  (CELL 14)
#            optionally add the global-context pass           (CELL 16 + 17)
#            save the PRE-NMS detections to NPZ               (CELL 18)
...
#    * psutil RAM guard        -> stops the shard cleanly and lets Phase 2 run on
#                                 what is finished; resubmit to continue. psutil
#                                 is optional: without it the guard is disabled.
#
#  MODES
...
def _host_ram_percent() -> Optional[float]:
    """
    Host RAM usage in percent, or None when psutil is unavailable.
    psutil is optional: without it the MEM_STOP_THRESHOLD_PCT guard is simply
    disabled and the run behaves exactly like E02_2.
    """
    try:
        import psutil
...
def _process_rss_gb() -> Optional[float]:
    """Resident memory of THIS shard process in GB, or None without psutil."""
    try:
        import psutil
...
SUPERVISION_HINT = (
    ...
    "have no internet: run './E01_2b_run_sam3.sh download' on a LOGIN node first, "
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="E01_2b - SAM3 single-exemplar-prompted Rumex detection with big "
                    "tiles and an optional global-context pass, cluster port of E01_2b.ipynb.",
    ...
    g.add_argument("--experiment-name", default="E01_2b",
    ...
    g.add_argument("--n-exemplars", type=int, default=1,
                   help="N_EXEMPLARS. E01_2b uses 1 (a single visual prompt: the "
                        "anchor itself). Set 3 to reproduce E02_2b's prompt scheme.")
    ...
    g.add_argument("--tile-size", type=int, default=2000,
                   help="TILE_SIZE. E01_2b uses 2000 (the E02_2 reference used 1000): "
                        "larger than the biggest measured plant.")
    g.add_argument("--overlap", type=int, default=800,
                   help="OVERLAP. E01_2b uses 800 (the E02_2 reference used 150): larger than "
                        "max plant width, so every plant is whole in some tile.")
    ...
    # -------------------------- evaluation ----------------------------------
    g = p.add_argument_group("evaluation (CELL 3 / CELL 22 / CELL 25)")
    ...
    # --------------------- global-context pass (CELL 3 / CELL 16) -----------
    g = p.add_argument_group("global-context pass (CELL 3 / CELL 16)")
    gp = g.add_mutually_exclusive_group()
    gp.add_argument("--global-pass", dest="add_global_context_pass",
                    action="store_true", default=True,
                    help="ADD_GLOBAL_CONTEXT_PASS = True (the notebook default): one "
                         "extra whole-image pass per anchor, pooled into the same "
                         "pre-NMS detection set as the tiles.")
    gp.add_argument("--no-global-pass", dest="add_global_context_pass",
                    action="store_false",
                    help="ADD_GLOBAL_CONTEXT_PASS = False: tiles only, which makes the "
                         "run directly comparable to E02_2 apart from the tile size.")
    g.add_argument("--global-downscale", type=int, default=2,
                   help="GLOBAL_DOWNSCALE: shrink factor for the global pass image. "
                        "2 on a 8192x5460 image means a 4096x2730 forward pass, which "
                        "is the single largest GPU allocation in this experiment.")

    # ---------------------------- runtime -----------------------------------
    g = p.add_argument_group("runtime")
    g.add_argument("--mem-stop-threshold-pct", type=float, default=70.0,
                   help="MEM_STOP_THRESHOLD_PCT: stop this shard CLEANLY when host RAM "
                        "exceeds this percentage, instead of being OOM-killed. The "
                        "finished runs are already on disk, so resubmitting resumes. "
                        "0 disables the guard. Needs psutil; without it, disabled.")
...
def compose_tile_with_exemplars(tile_img: Image.Image, crop_images: Sequence[Image.Image],
    ...
    Input : tile_img    - PIL image of the tile
            crop_images - list of PIL exemplar crops (1 for E01_2b, 3 for E02_2b)
...
def run_global_context_pass(runner: "Sam3Runner", image: Image.Image,
                            exemplar_crops: Sequence[Image.Image], run_seed: int,
                            args) -> dict:
    """
    ...
    (boxes, scores, fills), = runner.infer_batch([composed], [crop_boxes], args.threshold)
...
# =============================================================================
# Runs SAM3 for ONE anchor/exemplar configuration over ALL cached tiles (same as
# E02_2), and -- if ADD_GLOBAL_CONTEXT_PASS is on -- ALSO runs the extra
# downsampled whole-image pass from CELL 16 and appends its detections to the
# SAME pre-NMS pool, tagged with tile_id = -1 so they stay identifiable later
...
def run_anchor_over_tiles(runner: Sam3Runner, tile_cache: List[dict],
                          ...
                          run_seed: int, args):
    """
    Input : tile_cache     - list of tile dicts (CELL 11), built once per image
            full_image     - the open PIL image (also the input of the global pass)
            exemplar_crops - list of PIL crops used as visual prompts
            run_seed       - deterministic seed for the strip background sampling
    Output: (detections, used_global_pass)
            detections = dict of numpy arrays, all in ORIGINAL-IMAGE coordinates:
            boxes (N,4), scores (N,), fill_ratio (N,), tile_id (N,),
            tile_boxes (N,4)  <- the tile each detection came from
                                 (tile_id == -1, tile_boxes == the whole image
                                 extent, for detections from the global pass)
            These are the PRE-NMS detections (score >= SAM3_INFERENCE_THRESHOLD,
    ...
    # ---- optional extra whole-image pass, pooled into the SAME detection set --
    used_global_pass = False
    if args.add_global_context_pass:
        global_det = run_global_context_pass(runner, full_image, exemplar_crops,
                                             run_seed, args)
...
def run_inference(args, paths: Paths, records: List[ImageRecord]) -> None:
    ...
    # ---- host RAM guard (CELL 3: MEM_STOP_THRESHOLD_PCT) --------------------
    # TILE_SIZE=2000 tiles plus the global pass use far more host RAM than
    # E02_2's 1000px tiles, and on a shared node four shards compete for it.
    # Rather than being OOM-killed mid-write, the shard stops CLEANLY: whatever
    # is in the manifests stays valid and the next submission resumes from it.
    # NOTE: psutil reports NODE-wide usage, so all shards will trip at roughly
    # the same moment. That is intended - they all stop cleanly together.
    mem_guard = args.mem_stop_threshold_pct > 0 and _host_ram_percent() is not None
    if args.mem_stop_threshold_pct > 0 and not mem_guard:
        print("NOTE: psutil is not available -- the RAM guard is disabled.")
    stopped_early = False
    ...
    for img_idx, rec in enumerate(records, start=1):
        ...
        if mem_guard:
            mem_pct = _host_ram_percent()
            if mem_pct is not None and mem_pct > args.mem_stop_threshold_pct:
                print(f"RAM at {mem_pct:.0f}% (threshold {args.mem_stop_threshold_pct:g}%) "
                      f"-- stopping this shard cleanly before {image_id} to avoid a hard "
                      f"crash. Resubmit the job to resume from where it stopped.")
                stopped_early = True
                break
        ...
        for anchor_idx in range(n_anchors):
            ...
            if mem_guard:
                mem_pct = _host_ram_percent()
                if mem_pct is not None and mem_pct > args.mem_stop_threshold_pct:
                    print(f"RAM at {mem_pct:.0f}% mid-image at {image_id} "
                          f"anchor={anchor_idx} -- stopping this shard cleanly. Completed "
                          f"anchors are already saved; resubmit the job to resume.")
                    stopped_early = True
                    break
    ...
    if stopped_early:
        print("\n" + "!" * 78)
        print(f"SHARD {args.shard_index} STOPPED EARLY (host RAM above "
              f"{args.mem_stop_threshold_pct:g}%). This is a clean stop, not a crash:")
        print("  every finished run is in the manifest and the evaluation still runs on it.")
        print("  RESUBMIT THE JOB to continue -- or lower TILE_SIZE / use --no-global-pass.")
        print("!" * 78)
...
def dry_run(args, records: List[ImageRecord], my_records: List[ImageRecord]) -> None:
    ...
    passes = tiles + (1 if args.add_global_context_pass else 0)
    ...
    if args.add_global_context_pass:
        print(f"  + 1 global-context pass per anchor run at "
              f"{8192 // args.global_downscale}x{5460 // args.global_downscale} "
              f"(the largest single forward pass of the run)")
    print(f"  => ~{total_anchors * passes} SAM3 forward passes for those {len(sample)} images")
...
# =============================================================================
#  CELL 24 - OPERATING POINT
# =============================================================================
# Choose one confidence threshold, remove the predictions below it, then send the
# remaining predictions to the CELL 22 evaluator. This is what produces
...
def run_evaluation(args, paths: Paths) -> None:
    """PHASE 2: notebook CELL 20 ... CELL 31, in one process, no GPU."""
    import pandas as pd
    ...
    print(f"SAM3 inference threshold : {args.threshold} "
          f"(executed once per image x anchor x tile)")
...
def main() -> None:
    ...
    if args.evaluate_only:
        ...
        if not _check_pandas():
            print("\nERROR: pandas is required for the evaluation. See "
                  "'Problem B' in E01_2b_HOW_TO_RUN.md.")
            sys.exit(2)
    ...
    print("=" * 92)
    print(f" E01_2b SAM3 SINGLE-EXEMPLAR PIPELINE | experiment={exp} | "
    ...
    print(f" global pass    : {args.add_global_context_pass} "
          f"(downscale={args.global_downscale})")
    print(f" RAM guard      : "
          f"{('%g%%' % args.mem_stop_threshold_pct) if args.mem_stop_threshold_pct > 0 else 'off'}"
          f"{'' if _host_ram_percent() is not None else '  (psutil MISSING -> disabled)'}")
