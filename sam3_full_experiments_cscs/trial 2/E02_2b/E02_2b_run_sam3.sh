#!/bin/bash
#
# E02_2b_run_sam3.sh  ==>  dispatcher for experiment E02_2b
#     (SAM3 exemplar-prompted Rumex detection with BIG TILES (2000/800) and an
#      optional GLOBAL-CONTEXT PASS, cluster port of E02_2b.ipynb)
#
# E02_2b differs from the E02_2 reference in three ways: bigger tiles, the extra
# whole-image pass pooled into the same pre-NMS detection set, and a host-RAM
# guard that stops a shard cleanly instead of letting it be OOM-killed.
#
# Sets up the environment, decides where everything lives under $SCRATCH and
# launches E02_2b_infer_sam3.py.
#
# MODES
...
N_EXEMPLARS="${N_EXEMPLARS:-3}"                  # 3 = multiple prompts, 1 = single
USE_TILING="${USE_TILING:-1}"                    # 1 = overlapping tiles, 0 = whole image
TILE_SIZE="${TILE_SIZE:-2000}"                   # E02_2b: bigger than the E02_2 1000
OVERLAP="${OVERLAP:-800}"                        # E02_2b: bigger than the E02_2 150
...
try:
    ...
    print("  -> inference will refuse to start. Run './E02_2b_run_sam3.sh download' on a login node.")
try:
    import pandas
...
PY
        ...
        echo "      with: ./E02_2b_run_sam3.sh dryrun   (the PYTHON DEBUG block prints the result)."
        ;;
    ...
    run)
        ...
        if [[ ! -d "${HF_HOME}" || -z "$(ls -A "${HF_HOME}" 2>/dev/null)" ]]; then
            ...
            echo "         Run './E02_2b_run_sam3.sh download' on a login node first."
        fi
