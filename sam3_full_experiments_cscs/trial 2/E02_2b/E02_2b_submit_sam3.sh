#!/bin/bash -l
#
#  E02_2b_submit_sam3.sh  --  SLURM batch script for experiment E02_2b
...
#  E02_2b_run_sam3.sh; PHASE 2 (the offline evaluation) runs once at the end,
...
#SBATCH --job-name="E02_2b_sam3"
#SBATCH --output=E02_2b_sam3_%j.out
#SBATCH --error=E02_2b_sam3_%j.err
...
chmod +x "${SCRIPT_DIR}/E02_2b_run_sam3.sh"

# Defaults for this job; any of these can be overridden from the submitting shell,
# e.g.  DTYPE=float16 sbatch E02_2b_submit_sam3.sh
export EXPERIMENT_NAME="${EXPERIMENT_NAME:-E02_2b}"
...
# --- notebook CELL 3 configuration (E02_2b: 3 exemplars, big tiles + global pass) ---
export N_EXEMPLARS="${N_EXEMPLARS:-3}"
export USE_TILING="${USE_TILING:-1}"
export TILE_SIZE="${TILE_SIZE:-2000}"             # E02_2b: bigger tiles
export OVERLAP="${OVERLAP:-800}"                  # E02_2b: bigger overlap
...
# E02_2b_run_sam3.sh sets. Do NOT rely on the compute node reaching PyPI -- it cannot. If the
# image's transformers is too old for Sam3Model, build a dedicated image
# (E02_2b_HOW_TO_RUN.md, "Problem A").
srun \
    --environment=yolo26 \
    --container-workdir="$PWD" \
    --cpu-bind=cores \
    bash -c "${SCRIPT_DIR}/E02_2b_run_sam3.sh run"
