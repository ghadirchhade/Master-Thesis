# HOW TO RUN — Experiment E02_2b

What E02_2b is: SAM3 prompted with 3 exemplar crops, tiling ON with **big tiles**
...
**E02_2b vs E02_2** — same prompts, same strip composition, same filters, same frozen operating
point, same evaluation. Three things differ:
...
evaluation cell downstream is untouched, which is what keeps E02_2b comparable to E02_2.

This is the cluster port of `E02_2b.ipynb`. Like the notebook, it is a **two-phase** pipeline:
...
chmod +x E02_2b_run_sam3.sh E02_2b_submit_sam3.sh
ls -l
```
...
run, so E02_2b is **not** 2.5× faster than E02_2 just because it has fewer tiles. Use the smoke
...
It writes to a throwaway folder so it cannot pollute the real E02_2b results.

```bash
EXPERIMENT_NAME=E02_2b_smoke \
OUTPUT_DIR=$SCRATCH/experiments/sam3/_smoke_E02_2b \
NUM_GPUS=1 \
./E02_2b_run_sam3.sh run --limit-images 2
```
...
SAM3 ready. model device: cuda:0
  [E02_2b_smoke] shard0 run #1 | AGS_Multi_Rumex/2022.../DJI_0001 | anchor=0 (1/12) | prompt=0+7+3 | tiles=28 | global_pass=True | pre-NMS detections=41 | 47.9s
...
...
```bash
cd $HOME/2025-OverneyTechnologiesProject/7_cscs_experiments/sam3
sbatch E02_2b_submit_sam3.sh
```

**You should see:** `Submitted batch job 1234567`

That is it. No arguments needed — E02_2b is the default. The job asks for 1 node, 4 GPUs,
...
tail -f E02_2b_sam3_1234567.out                      # everything, live
tail -f $SCRATCH/experiments/sam3/E02_2b/shard0.log  # just GPU 0
```

Count how many runs are finished so far:

```bash
cat $SCRATCH/experiments/sam3/E02_2b/raw_detections/runs_manifest_E02_2b_shard*.csv \
  | grep -c E02_2b
ls $SCRATCH/experiments/sam3/E02_2b/raw_detections/*.npz | wc -l
```
...
Everything lands in `$SCRATCH/experiments/sam3/E02_2b/`:

```bash
ls -R $SCRATCH/experiments/sam3/E02_2b/ | head -40
```
...
| `raw_detections/runs_manifest_E02_2b_shard*.csv` | which runs are finished; this is what resume reads |
| `run_config_E02_2b.json` | every parameter used, for the thesis appendix |
| `shard0..3.log` | per-GPU logs |

Look at the headline table:

```bash
cd $SCRATCH/experiments/sam3/E02_2b/metrics
...
```
Resuming: 4821 run(s) already finished for E02_2b; skipped.
```
...
OUTPUT_DIR=$SCRATCH/experiments/sam3/E02_2b \
./E02_2b_run_sam3.sh evaluate
```
...
2. **`BATCH_SIZE=8 sbatch E02_2b_submit_sam3.sh`** — the notebook used 4 because a T4 has 16 GB;
...
3. **`--max-anchors-per-image 5`** — instead of every ground-truth box taking a turn as the
   ...
   in the thesis. Pass it after the mode: `./E02_2b_run_sam3.sh run --max-anchors-per-image 5`.
4. **More GPUs** — needs a job array; ask and I will write one.

## The two E02_2b-specific features
...
```bash
ADD_GLOBAL_CONTEXT_PASS=0 EXPERIMENT_NAME=E02_2b_notglobal \
OUTPUT_DIR=$SCRATCH/experiments/sam3/E02_2b_notglobal sbatch E02_2b_submit_sam3.sh
```
...
```bash
MEM_STOP_THRESHOLD_PCT=85    sbatch E02_2b_submit_sam3.sh   # allow more RAM
ADD_GLOBAL_CONTEXT_PASS=0    sbatch E02_2b_submit_sam3.sh   # drop the biggest allocation
NUM_GPUS=2                   sbatch E02_2b_submit_sam3.sh   # 2 shards instead of 4
MEM_STOP_THRESHOLD_PCT=0     sbatch E02_2b_submit_sam3.sh   # disable (risks an OOM kill)
```

Without `psutil` the guard is simply off and the run behaves like E02_2 in that respect.

## Comparing E02_2b against E02_2

The two write to different folders and share every evaluation setting, so their headline tables
stack directly:

```bash
head -1 $SCRATCH/experiments/sam3/E02_2/metrics/experiment_summary.csv
tail -n +2 -q $SCRATCH/experiments/sam3/E02_2/metrics/experiment_summary.csv
tail -n +2 -q $SCRATCH/experiments/sam3/E02_2b/metrics/experiment_summary.csv
```

`tile_size`, `overlap` and `add_global_context_pass` are columns in that CSV, so each row
carries its own configuration. Two variables changed at once between E02_2 and E02_2b, so a
single difference in F1 cannot be attributed to either one alone — run the
`ADD_GLOBAL_CONTEXT_PASS=0` variant above if you need to separate them.
...
then change the one line at the bottom of `E02_2b_submit_sam3.sh`:
...
```bash
THRESHOLD=0.05 EXPERIMENT_NAME=E02_2b_lowconf \
OUTPUT_DIR=$SCRATCH/experiments/sam3/E02_2b_lowconf sbatch E02_2b_submit_sam3.sh
```
...
| Host RAM fills up | `./E02_2b_run_sam3.sh run --no-cache-tiles` — tiles are then cropped on demand instead of all being held in RAM (`CACHE_TILES_IN_MEMORY = False` in the notebook) |
...
```bash
DTYPE=float16           sbatch E02_2b_submit_sam3.sh   # exactly what the notebook used
BATCH_SIZE=8            sbatch E02_2b_submit_sam3.sh   # faster on a big GPU
THRESHOLD=0.05          sbatch E02_2b_submit_sam3.sh   # save more detections for the AP curve
BEST_CONFIDENCE=0.30    sbatch E02_2b_submit_sam3.sh   # different operating point
BEST_NMS_IOU=0.50       sbatch E02_2b_submit_sam3.sh   # different NMS
N_EXEMPLARS=1           sbatch E02_2b_submit_sam3.sh   # single visual prompt instead of 3
USE_TILING=0            sbatch E02_2b_submit_sam3.sh   # whole image as one tile
ADD_GLOBAL_CONTEXT_PASS=0 sbatch E02_2b_submit_sam3.sh # tiles only, no whole-image pass
GLOBAL_DOWNSCALE=4      sbatch E02_2b_submit_sam3.sh   # cheaper/coarser global pass
TILE_SIZE=1500          sbatch E02_2b_submit_sam3.sh   # smaller tiles if you hit OOM
MEM_STOP_THRESHOLD_PCT=85 sbatch E02_2b_submit_sam3.sh # allow more host RAM
NUM_GPUS=2              sbatch E02_2b_submit_sam3.sh   # fewer GPUs
DATASET_ROOT=/some/path sbatch E02_2b_submit_sam3.sh   # dataset elsewhere
```
